...
if ('delete' === $method && '' !== $iUserId) {
    if (!$userObj->hasPermission('delete-users')) {
        ...
        if (SITE_TYPE !== 'Demo') {
            $query = "UPDATE register_user SET eStatus = 'Deleted' , vPhone = concat(vPhone, '(Deleted)') WHERE iUserId = '".$iUserId."'";
            $obj->sql_query($query);
            if (1 === $hardDelete) {
...
if ('' !== $iUserId && '' !== $status) {
    if (!$userObj->hasPermission('update-status-users')) {
        ...
        if (SITE_TYPE !== 'Demo') {
            // --------------------- user deleted duplicate check --------------------
            $checkUserDeleted = $obj->MySQLSelect("SELECT vPhone FROM register_user WHERE eStatus = 'Deleted' AND iUserId='".$iUserId."'");

            if (!empty($checkUserDeleted)) {
                $mobile = clearPhone($checkUserDeleted[0]['vPhone']);
                $checkUserDeleted = $obj->MySQLSelect("SELECT vPhone FROM register_user WHERE eStatus != 'Deleted' AND iUserId != '".$iUserId."' AND vPhone='".$mobile."'");
                ...
                $query = "UPDATE register_user SET vPhone = '".$mobile."' WHERE iUserId = '".$iUserId."'";
...
function removedRiderData($riderId): void
{
    ...
    if ('Yes' === $ENABLE_CORPORATE_PROFILE) {
