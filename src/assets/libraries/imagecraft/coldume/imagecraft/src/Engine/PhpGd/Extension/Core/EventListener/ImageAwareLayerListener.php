...
class ImageAwareLayerListener implements EventSubscriberInterface
{
     ...
     * @param PhpGdEvent
     */
    public function initImcUri(PhpGdEvent $event): void
    {
        ImcStream::register();
     ...
     * @param PhpGdEvent
     */
    public function initFilePointer(PhpGdEvent $event): void
    {
        $layers = $event->getLayers();
        foreach ($layers as $key => $layer) {
            if (!$layer instanceof ImageAwareLayerInterface) {
            ...
            if ($layer->has('image.imc_uri')) {
                $fp = fopen($layer->get('image.imc_uri'), 'r');
     ...
     * @param PhpGdEvent
     */
    public function initImageInfo(PhpGdEvent $event): void
    {
        $layers = $event->getLayers();
     ...
     * @param PhpGdEvent
     */
    public function initFinalDimensions(PhpGdEvent $event): void
    {
        $layers = $event->getLayers();
        foreach ($layers as $layer) {
            if (!$layer instanceof ImageAwareLayerInterface) {
     ...
     * @param PhpGdEvent
     */
    public function termFilePointer(PhpGdEvent $event): void
    {
        $layers = $event->getLayers();
        foreach ($layers as $layer) {
            if (!$layer instanceof ImageAwareLayerInterface) {
     ...
     * @param PhpGdEvent
     */
    public function termImcUri(PhpGdEvent $event): void
    {
        $layers = $event->getLayers();
        foreach ($layers as $key => $layer) {
            if (!$layer instanceof ImageAwareLayerInterface) {
