...
if ($type == "getDriverSubscriptionHistory") { //returns all plans of the driver
    ...
    //CONCAT('$currencySymbol',FORMAT(d.fPrice * $userCurrencyRatio,2)) as fPlanPrice,
    $getField = "d.eSubscriptionStatus, d.vPlanName, d.vPlanDescription,d.vPlanPeriod,d.ePlanValidity,FORMAT(d.fPrice * $userCurrencyRatio,2) as fPlanPrice,
            CASE 
    ...
    foreach ($getDriverSubscription as $key => $value) {
        $getDriverSubscription[$key]['fPlanPrice'] = formateNumAsPerCurrency($value['fPlanPrice'], $currencycode);
    }
...
if ($type == "getSubscriptionPlans") { //returns all plans which are addd by admin and also shows the details of driver wheather he will be subscribed or not, if not used plans then it shows N/A
    ...
    $getField = "p.iDriverSubscriptionPlanId, 
            ...
            END AS PlanDuration,
            FORMAT(p.fPrice * $userCurrencyRatio,2) as fPlanPrice,
...
if($type=='SubscribePlan') { 
    ...
    if($fPrice==0 || empty($fPrice)) {
        // Commented by HV to allow zero price subscription plan
        /*$returnArr['Action'] = 0;
        $returnArr['message'] = "LBL_NO_SUBSCRIPTION_PLANS";
        setDataResponse($returnArr);*/
    ...
    $user_available_balance_cur = $WALLET_OBJ->FetchMemberCurrencyWalletBalance($iDriverId, "Driver");
    ...
    if($isWallet=='Yes' && $payStatus == "") {
        ...
        if($user_available_balance_cur>=$fPrice_cur) { 
            $insid = $WALLET_OBJ->PerformWalletTransaction($iDriverId, 'Driver', $walletbalance, 'Debit', 0, 'Subscription', $WalletSubscriptionTxt, 'Unsettelled', $date);
            if(empty($insid)) {
                $returnArr['Action'] = 0;
                $returnArr['message'] = "LBL_NO_SUBSCRIPTION_PLANS";
                setDataResponse($returnArr);
    ...
    //if($user_available_balance>0 && $isWallet=='Yes') {
        ...
        //echo $walletbalance_cur."=====".$walletbalance; exit;
    ...
    /*Added By PM On 09-12-2019 For Flutterwave Code Start*/
        $Squery = "SELECT vStripeToken,vStripeCusId,vCreditCard,vBrainTreeToken,vFlutterWaveToken,vOmiseToken,vPaymayaToken FROM register_driver WHERE iDriverId = '" . $iDriverId . "'";
    ...
    $vPaymayaToken = $Userdata[0]['vPaymayaToken'];
    /*Added By PM On 09-12-2019 For Flutterwave Code Start*/
        $vFlutterWaveToken = $Userdata[0]['vFlutterWaveToken'];
    /*Added By PM On 09-12-2019 For Flutterwave Code Start*/
        $vOmiseToken = $Userdata[0]['vOmiseToken'];
        $ePaymentOption = 'Card';
		$IsChargeCustomer ="Yes";
	...
	/*Added By PM On 09-12-2019 For Flutterwave Code Start*/
        ...
        if ($vOmiseToken != "" && $APP_PAYMENT_METHOD=="Omise") {		
			$vCreditCard = $Userdata[0]['vCreditCard'];
			$IsChargeCustomer = ($vCreditCard != "") ? "Yes" : "No";
		}
	/*Added By PM On 09-12-2019 For Flutterwave Code End*/
        ...
        $iMemberId = $iDriverId;
        $UserType = "Driver";
        $userPaymentInfoSql = " (SELECT tCardNum FROM user_payment_info WHERE iMemberId = ".$iMemberId." AND eUserType = '".$UserType."' AND vPaymentMethod = '".$APP_PAYMENT_METHOD."' AND eStatus = 'Active' AND eDefault = 'Yes') as tCardNum, (SELECT iPaymentInfoId FROM user_payment_info WHERE iMemberId = ".$iMemberId." AND eUserType = '".$UserType."' AND vPaymentMethod = '".$APP_PAYMENT_METHOD."' AND eStatus = 'Active' AND eDefault = 'Yes') as iPaymentInfoId ";
        if ($UserType == "Passenger") {
            $sql = "SELECT tSessionId, ".$userPaymentInfoSql." FROM `register_user` WHERE iUserId='" . $iMemberId . "'";
        } else {
            $sql = "SELECT tSessionId, ".$userPaymentInfoSql." FROM `register_driver` WHERE iDriverId='" . $iMemberId . "'";
        }

        $userData = $obj->MySQLSelect($sql);

        //if($userData[0]['iPaymentInfoId'] == "")
        {
            $iMemberId = $iDriverId;
            $UserType = "Driver";
            ...
            $extraParams = "tSessionId=".$userData[0]['tSessionId']."&GeneralMemberId=".$iMemberId."&GeneralUserType=".$UserType."&iDriverSubscriptionPlanId=".$iDriverSubscriptionPlanId."&isWallet=".$isWallet."&isCard=".$isCard."&isUpgrade=".$isUpgrade."&AMOUNT=".$cardbalance."&PAGE_TYPE=SUBSCRIBE_PLAN&SYSTEM_TYPE=APP&IS_RETURN_RESULT=Yes";
        ...
        if ($IsChargeCustomer=='Yes') {
            ...
            if ($result['Action'] == "1") 
            {
                ...
                $data_payments['iOrderId'] = $iDriverSubscriptionPlanId;
                $data_payments['eEvent'] = "SubscribePayment";
                $obj->MySQLQueryPerform("payments", $data_payments, 'update', $where_payments);
                ...
                updateDriverSubscription($iDriverSubscriptionPlanId,$iDriverId,$fPrice,$insid,$payment_id);
            }
            else{
                $returnArr = $result;
    ...
    /*if (isset($result['status']) && $result['status'] == "succeeded" && $result['paid'] == "1" || $status == "success" || $payStatus == "succeeded") { //|| $fNetTotal == 0) {
        ...
        if ($ChargeidArrId == 0) {
                ...
                if ($APP_PAYMENT_METHOD=="Omise") {
                    $payment_arr['OMISE_SECRET_KEY'] = $OMISE_SECRET_KEY;
                    $payment_arr['OMISE_PUBLIC_KEY'] = $OMISE_PUBLIC_KEY;
                }
...
if($type=='CancelSubscription') {
    ...
    if(empty($id)) {
        ...
        $mail = $COMM_MEDIA_OBJ->SendMailToMember('DRIVER_SUBSCRIPTION_CANCEL',$getMaildata);
...
function checkDriverSubscribed($iDriverId,$selectedVehicleId=-1) { //its used when driver online call this function to chk whether he is subscribed or not
    ...
    $selExpired = "SELECT iDriverSubscriptionDetailsId,iDriverId,tExpiryDate >= '$curdate' AS tExpiryDate, datediff(tExpiryDate,'$curdate') AS daysRemain,eSubscriptionStatus FROM $tblDetails WHERE iDriverId = $iDriverId"; 
    $dataExpired = $obj->MySQLSelect($selExpired);
    ...
    if($getDriverSubscription[0]['cnt']==0) {
        ...
        if(($Data_Car[0]['iSelectedVehicleId']=='' || $Data_Car[0]['iSelectedVehicleId']==0) && $selectedVehicleId==-1) {
            //My Services portion is for checking uberx services active or not.
            if($Data_Car[$i]['eType'] == "UberX") {
                    ...
                    if (count($db_cartype) > 0) {
                        for ($j = 0; $j < count($db_cartype); $j++) {
                            ...
                            if ($eVehicleType == "DeliverAll") {
                                $pendingSubscription_mix = 1;//becoz when ride vehicle is not active but deliverall vehicle is active so tell the user that if do you want ride then subscription
                            } else {
            ...
            while (count($Data_Car) > $i) {
                if((($Data_Car[0]['iSelectedVehicleId'] == $Data_Car[$i]['iDriverVehicleId'] && $selectedVehicleId==-1) || $selectedVehicleId!=-1 && $selectedVehicleId == $Data_Car[$i]['iDriverVehicleId']) || $Data_Car[$i]['eType'] == "UberX")  {
                    $vCarType = $Data_Car[$i]['vCarType'];
                    if ($vCarType != "") {
                        $sql = "SELECT iVehicleTypeId,eType  FROM `vehicle_type` WHERE `iVehicleTypeId` IN ($vCarType)";
                        $db_cartype = $obj->MySQLSelect($sql);
...
function updateDriverSubscription($iDriverSubscriptionPlanId,$iDriverId,$balance,$walletId = 0,$paymentId = 0) {
    ...
    //Added By HJ On 13-08-2020 For Solved 1970-01-01 Issue  Start
    $vPlanPeriod = 1;
    $PlanTypeTitle = "Days";
    if(count($dbPlans) > 0){
    ...
    //Added By HJ On 13-08-2020 For Solved 1970-01-01 Issue  End
