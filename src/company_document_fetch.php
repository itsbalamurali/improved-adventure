...
if ('company' === $user && 'DeliverAll' === $eSystem) {
...
if ('' === $db_user[0]['document']) {
    $vName = $db_user1[0]['document'];
} else {
    $vName = $db_user[0]['document'];
}
?>
<div class="upload-content">
    ...
    <form class="form-horizontal frm6" id="frm6" method="post" enctype="multipart/form-data" action="dashboard.php?id=<?php echo $id[1]; ?>&master=<?php echo $id[0]; ?> " name="frm6">
        ...
        <input type="hidden" name="doc_path" value =" <?php if ('company' === $user) {
                    ...
                    <div class="fileupload-preview thumbnail" style="width: 100%; height: 150px; ">
                        <?php if ('' === $db_user[0]['doc_file']) {
                            ...
                            if ('is_image' === $file_ext) {
                                ...
                                <img src = "<?php echo $tconfig.'/'.$id[1].'/'.$db_user[0]['doc_file']; ?>" style="width:100%;" alt ="<?php echo $db_user[0]['doc_name']; ?> not found"/>
                            <?php } else {
                                if ('company' === $user) {
                                    $tconfig = $tconfig['tsite_upload_compnay_doc'];
                                } else {
                                ...
                                ?>
                                <a href="<?php echo $tconfig.'/'.$id[1].'/'.$db_user[0]['doc_file']; ?>" target="_blank"><?php echo $db_user[0]['doc_name']; ?></a>
                            <?php } ?>
                        <?php } ?>
                    </div>
                    <div>
                        <span class="btn btn-file btn-success"><span class="fileupload-new"><?php echo $langage_lbl['LBL_UPLOAD']; ?> <?php echo $vName; ?> <?php echo $langage_lbl['LBL_PHOTO']; ?></span>
                            <span class="fileupload-exists"><?php echo $vName; ?> <?php echo $langage_lbl['LBL_CHANGE']; ?></span>
                            <input type="file" name="driver_doc" /></span>

                        <a href="#" class="btn btn-danger fileupload-exists" data-dismiss="fileupload"><?php echo $vName; ?> <?php echo $langage_lbl['LBL_REMOVE_TEXT']; ?></a>
        ...
        <?php if ('yes' === $db_user[0]['ex_status'] || 'yes' === $db_user1[0]['ex_status']) { ?>

        <h5><b><?php echo $langage_lbl['LBL_EXP_DATE']; ?></b></h5>
        <div class="col-lg-13 exp-date">
            <div class="input-group input-append date" id="dp122">
                <input class="form-control" type="text" name="dLicenceExp" value="<?php if ('' === $db_user[0]['ex_date']) {
                    echo '0000-00-00';
                } else {
                    echo $db_user[0]['ex_date'];
                }?>"
                placeholder="<?php if ('' === $db_user[0]['ex_date']) {
                    echo '0000-00-00';
                } else {
                    echo $db_user[0]['ex_date'];
            ...
            if (('' === $db_user[0]['ex_date'] || $Today >= $allowDate) || 'No' === $SET_DRIVER_OFFLINE_AS_DOC_EXPIRED) {
                $disable = '';
                if ('Inactive' === $db_user[0]['eStatus'] && $db_user[0]['statusUpdatedCount'] > 1) {
                    $disable = 'disabled';
                }
                ?>
        ...
        <input type="submit" class="save save11" name="save" value="<?php echo $langage_lbl['LBL_Save']; ?>" <?php echo $disable; ?>>

        <input type="button" class="cancel cancel11" data-dismiss="modal" name="cancel" value="<?php echo $langage_lbl['LBL_CANCEL_TXT']; ?>">
