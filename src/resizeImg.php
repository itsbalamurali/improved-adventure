...
require_once('assets/libraries/FFMpeg/autoload.php');
...
if (!defined('FILE_CACHE_PARENT_DIRECTORY'))
    define('FILE_CACHE_PARENT_DIRECTORY', 'webimages/resizeImage'); 

if(!is_dir(FILE_CACHE_PARENT_DIRECTORY)) {
    mkdir(FILE_CACHE_PARENT_DIRECTORY);
}
// -------------------------------------------------------------
    ...
    public function __construct() {
        ...
        if (FILE_CACHE_DIRECTORY) {
            if (!is_dir(FILE_CACHE_DIRECTORY)) {
                ...
                @mkdir(FILE_CACHE_DIRECTORY, 0777); // NM Add 0777 to give permission
                if (!is_dir(FILE_CACHE_DIRECTORY)) {
        ...
        try{
            ...
            if(!empty($imgFileInfo->getExtension()) && ($imgFileInfo->getExtension() == "gif" || $imgFileInfo->getExtension() == "GIF")){
                ...
                echo $image;
                exit;
            }else if(!empty($imgFileInfo->getExtension()) && ($imgFileInfo->getExtension() == "mp4" || $imgFileInfo->getExtension() == "MP4")){
                
                $dir_path_file = dirname(__FILE__)."/";
                
                $cache_dir_gif = FILE_CACHE_DIRECTORY;
              
                $localImageGIFPath = $dir_path_file."webimages/upload/upload_videos/".basename($this->src, ".".$imgFileInfo->getExtension()).'.'.$imgFileInfo->getExtension(); 
                
                $sec = 2;
                $movie = $localImageGIFPath;
                $thumbnail = $dir_path_file.'webimages/upload/thumbnail.jpg';
                ...
                $video  = $dir_path_file."webimages/upload/upload_videos/". basename($this->src, ".".$imgFileInfo->getExtension()).'.'.$imgFileInfo->getExtension();
            
                $thumbnail = $dir_path_file."webimages/upload/upload_videos/".basename($this->src, ".".$imgFileInfo->getExtension()).'.jpg';  
            
                $thumbnail_video = $localImageGIFPath; 
                $thumbSize       = '150x150';
            
                $cmd = "ffmpeg -i $video -deinterlace -an -ss 1 -t 00:00:01 -r 1 -y -vcodec mjpeg -f mjpeg  $thumbnail 2>&1"; 
                $cmd1 = "ffmpeg -i $video -s $thumbSize -strict -2  $thumbnail_video 2>&1"; 
            
                exec($cmd, $output, $retval);
                exec($cmd1, $output, $retval);
