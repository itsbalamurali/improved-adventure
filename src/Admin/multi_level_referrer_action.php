...
if ('Driver' === $type) {
    $q1 = "SELECT rd.vName,rd.vLastName,concat(rd.vName, ' ' ,rd.vLastName) as OrgName,rd.eRefType,rd.iDriverId,rd.iRefUserId,rd.dRefDate FROM register_driver as rd WHERE rd.iRefUserId = '".$id."' AND rd.eRefType = '".$type."'";
    $result_driver = $obj->MySQLSelect($q1);

    $q2 = "SELECT ru.vName,ru.vLastName,concat(ru.vName, ' ' ,ru.vLastName) as OrgName,ru.eRefType, ru.iUserId,ru.iRefUserId,ru.dRefDate FROM register_user as ru WHERE ru.iRefUserId = '".$id."' AND ru.eRefType = '".$type."'";
    $result_rider = $obj->MySQLSelect($q2);
} else {
    $q3 = "SELECT rd1.vName,rd1.vLastName,concat(rd1.vName, ' ' ,rd1.vLastName) as OrgName,ru.eRefType,rd1.iDriverId,rd1.iRefUserId,rd1.dRefDate FROM register_user as ru LEFT JOIN register_driver as rd1 on rd1.iRefUserId=ru.iUserId WHERE rd1.iRefUserId = '".$id."' AND rd1.eRefType = '".$type."'";
    $result_driver = $obj->MySQLSelect($q3);

    $q4 = "SELECT ru.vName,ru.vLastName,concat(ru.vName, ' ' ,ru.vLastName) as OrgName,ru.eRefType, ru.iUserId,ru.iRefUserId,ru.dRefDate FROM register_user as ru LEFT JOIN register_user as ru1 on ru1.iUserId=ru.iRefUserId WHERE ru.iRefUserId = '".$id."' AND ru.eRefType = '".$type."'";
    $result_rider = $obj->MySQLSelect($q4);
}
$referrerDataNew = array_merge($result_driver, $result_rider);

$referrerSql = "SELECT iUserId,eUserType,fromUserId,fromUserType,dDate,iBalance FROM user_wallet WHERE iUserId = {$id} AND eUserType = '".$type."' AND eFor = 'Referrer' AND fromUserId > 0";
$referrerData = $obj->MySQLSelect($referrerSql);

$all_referrer_data = [];
foreach ($referrerData as $referrer) {
    if ('Rider' === $referrer['fromUserType']) {
    ...
    foreach ($tReferrerInfo as $tReferrer) {
        ...
        $fromRefData[] = ['name' => $tRefData[0]['referrer_name'], 'iMemberId' => $tReferrer['iMemberId'], 'eUserType' => $tReferrer['eUserType']];
    }
                                             ...
                                             <div class="panel-body" style="padding:0">
                                                  <div class="table-responsive">
                                                       <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                            <thead>
                                                                <tr>
																	<th width="45%">Referred Member Name</th>
																	<th width="25%" style="text-align:center;">Member Type</th>
																	<th width="35%" style="text-align:center;" >Date of Referred</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
...
if ($count > 0) {
    for ($i = 0; $i < count($referrerDataNew); ++$i) { ?>
																	 <tr class="gradeA">
                                    									...
                                    									<td ><?php if (!empty($referrerDataNew[$i]['iDriverId'])) { ?>
                                                               <?php if ($userObj->hasPermission('view-providers')) { ?><a href="javascript:void(0);" onClick="show_driver_details('<?php echo $referrerDataNew[$i]['iDriverId']; ?>')" style="text-decoration: underline;"><?php } ?><?php echo clearName($referrerDataNew[$i]['OrgName']); ?><?php if ($userObj->hasPermission('view-providers')) { ?></a> <?php } ?>
 															    <?php } else { ?>
																  <?php if ($userObj->hasPermission('view-users')) { ?><a href="javascript:void(0);" onClick="show_rider_details('<?php echo $referrerDataNew[$i]['iUserId']; ?>')" style="text-decoration: underline;"><?php } ?><?php echo clearName($referrerDataNew[$i]['OrgName']); ?><?php if ($userObj->hasPermission('view-users')) { ?></a><?php } ?>
																	<?php } ?></td>
																		<?php
                $time = strtotime($referrerDataNew[$i]['dRefDate']);
        $myFormatForView = date('jS F Y', $time);
        ?>
																		<td align="center"><?php echo ($referrerDataNew[$i]['iDriverId'] > 0 && !empty($referrerDataNew[$i]['iDriverId'])) ? $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN'] : $langage_lbl_admin['LBL_RIDER_NAME_TXT_ADMIN']; ?></td>
																		<td align="center"><?php echo $myFormatForView; ?></td>
																	 </tr>
						                ...
						                <div class="panel-body" style="padding:0">
                                                <div class="table-responsive">
                                                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                        <thead>
                                                            <tr>
						                                    <th width="45%"><?php echo $langage_lbl_admin['LBL_TRIP_TXT_ADMIN']; ?> Done by</th>
						                                    <th width="25%" style="text-align:center;">Amount Earned</th>
						                                   	<th width="35%" style="text-align:center;">Date of Received</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
						                                <?php $total_ref_amount = 0;
if (count($all_referrer_data) > 0) {  ?>
						                                <?php $countRef = 1;
    foreach ($all_referrer_data as $referrer_data) {
        ...
        ?>
	                                                                	...
	                                                                	<div class="panel-group pull-left full-width" id="accordion<?php echo $countRef; ?>" role="tablist" aria-multiselectable="true">
																		    <div class="panel panel-default">
																		        <div class="panel-heading pull-left full-width cursor-pointer referrer-list collapsed" role="tab" id="heading<?php echo $countRef; ?>" data-toggle="collapse" data-parent="#accordion<?php echo $countRef; ?>" href="#collapse<?php echo $countRef; ?>" aria-expanded="true" aria-controls="collapse<?php echo $countRef; ?>">
																	                ...
																	                <?php echo clearName($referrer_data['fromUserName']); ?> (<?php echo ('Rider' === $referrer_data['fromUserType']) ? $langage_lbl_admin['LBL_RIDER_NAME_TXT_ADMIN'] : $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?>)
																		                </a>
																		            </h4>
																		        ...
																		        <div id="collapse<?php echo $countRef; ?>" class="panel-collapse collapse pull-left full-width" role="tabpanel" aria-labelledby="heading<?php echo $countRef; ?>">
																		            <div class="panel-body">
																						...
																						<li class="list-group-item">
																							<?php if ('Rider' === $fromRef['eUserType']) { ?>
																							<?php if ($userObj->hasPermission('view-users')) { ?><a href="javascript:void(0);" onClick="show_rider_details('<?php echo $fromRef['iMemberId']; ?>')" style="text-decoration: underline;"><?php } ?><?php echo clearName($fromRef['name']); ?><?php if ($userObj->hasPermission('view-users')) { ?></a><?php } ?>
																							<?php } else { ?>
																							<?php if ($userObj->hasPermission('view-providers')) { ?><a href="javascript:void(0);" onClick="show_driver_details('<?php echo $fromRef['iMemberId']; ?>')" style="text-decoration: underline;"><?php } ?><?php echo clearName($fromRef['name']); ?><?php if ($userObj->hasPermission('view-providers')) { ?></a> <?php } ?>
																							<?php } ?> (<?php echo ('Rider' === $fromRef['eUserType']) ? 'User' : 'Provider'; ?>)</li>
																		                <?php } ?>
																		                </ul>
																		            </div>
																		        </div>
						                                        ...
						                                        <td align="center"><?php echo ($referrer_data['iBalance'] > 0) ? formateNumAsPerCurrency($referrer_data['iBalance'], '') : '--'; ?></td>
	                                                                <td align="center"><?php echo $referrer_data['referrerDate']; ?></td>
	                                                            </tr>
                                                            <?php ++$countRef;
    } ?>
