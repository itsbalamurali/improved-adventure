...
$tsite_upload_compnay_doc = $tconfig['tsite_upload_compnay_doc'];
if ('tracking_company' === $user_type) {
    $script = 'TrackServiceCompany';
    $doc_usertype = 'trackcompany';
    $tsite_upload_compnay_doc = $tconfig['tsite_upload_track_company_doc'];
}
...
$sql1 = "SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.doc_name ,dm.ex_status,dm.status, dl.doc_masterid masterid_list ,dl.ex_date,dl.doc_file ,dl.req_date,dl.doc_id,dl.req_file,  dl.status, dm.eType FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='".$_REQUEST['id']."' and doc_usertype='".$doc_usertype."') dl on dl.doc_masterid=dm.doc_masterid
    where dm.doc_usertype='".$doc_usertype."' and dm.status='Active' and (dm.country ='".$iCompanyId[0]['vCountry']."' OR dm.country ='All') {$eTypeQuery} ORDER BY `iDisplayOrder`";
...
$sql2 = "SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.doc_name ,dm.ex_status,dm.status, dl.doc_masterid masterid_list ,dl.ex_date,dl.doc_file ,dl.req_date,dl.doc_id,dl.req_file,  dl.status, dm.eType FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='".$_REQUEST['id']."' and doc_usertype='".$doc_usertype."') dl on dl.doc_masterid=dm.doc_masterid
   where dl.req_date != '' AND dl.req_date != '0000-00-00' and dm.doc_usertype='".$doc_usertype."' and dm.status='Active' and (dm.country ='".$iCompanyId[0]['vCountry']."' OR dm.country ='All') {$eTypeQuery} ORDER BY `iDisplayOrder`";
...
if ($action = 'document' && isset($_POST['doc_type'])) {
    ...
    if ('' === $image_name) {
        if ('' !== $expDate) {
            ...
            header('location:company_document_action.php?success=1&id='.$_REQUEST['id'].'&var_msg='.$var_msg.'&user_type='.$user_type);

            exit;
        }
        $var_msg = 'Please upload valid file format for Image. Valid formats are pdf,doc,docx,jpg,jpeg,gif,png';
        header('location:company_document_action.php?success=3&id='.$_REQUEST['id'].'&var_msg='.$var_msg.'&user_type='.$user_type);
    ...
    if ('' !== $_FILES['company_doc']['name']) {
        ...
        $tbl = 'document_list';
        $sql = 'select doc_id from  '.$tbl."  where doc_userid='".$_REQUEST[id]."' and doc_usertype='".$doc_usertype."'  and doc_masterid=".$_REQUEST['doc_type'];
        $db_data = $obj->MySQLSelect($sql);
        $q = 'INSERT INTO ';
        $where = '';
        if (count($db_data) > 0) {
            $query = 'UPDATE `'.$tbl."` SET `doc_file`='".$vImage."' , `ex_date`='".$expDate."' WHERE doc_userid='".$_REQUEST[id]."' and doc_usertype='".$doc_usertype."'  and doc_masterid=".$_REQUEST['doc_type'];
        } else {
            $query = ' INSERT INTO `'.$tbl.'` ( `doc_masterid`, `doc_usertype`, `doc_userid`, `ex_date`, `doc_file`, `status`, `edate`) '.'VALUES '."( '".$_REQUEST['doc_type']."', '".$doc_usertype."', '".$_REQUEST['id']."', '".$expDate."', '".$vImage."', 'Inactive', CURRENT_TIMESTAMP)";
        }
        ...
        save_log_data($_SESSION['sess_iUserId'], $_REQUEST['id'], 'company', 'Document Company', $vNocPath);
        header('location:company_document_action.php?success=1&id='.$_REQUEST['id'].'&var_msg='.$var_msg.'&user_type='.$user_type);
...
<head>
    <meta charset="UTF-8"/>
    <title><?php echo $SITE_NAME; ?> | Company <?php echo $action; ?></title>
    ...
    <div id="content">
        <div class="inner">
            <div class="row">
                ...
                $file = 'company.php';
if ('tracking_company' === $user_type) {
    $file = 'track_service_company.php';
} ?>
                    <a class="back_link"
                    ...
                    <div class="row company-document-action">
                        <?php for ($i = 0; $i < $count_all; ++$i) { ?>
                            <div class="col-lg-3">
                                <div class="panel panel-default upload-clicking">
                                    <div class="panel-heading"><?php echo $iCompanyIddoc[$i]['doc_name']; ?>
                                    </div>
                                    <div class="panel-body">
                                        <?php if (('' !== $iCompanyIddoc[$i]['doc_file'] && file_exists('../webimages/upload/documents/company/'.$_REQUEST['id'].'/'.$db_userdoc[$i]['doc_file'])) || ('' !== $iCompanyIddoc[$i]['doc_file'] && file_exists('../webimages/upload/documents/TrackCompany/'.$_REQUEST['id'].'/'.$db_userdoc[$i]['doc_file']))) { ?>
                                            <?php
                        $file_ext = $UPLOAD_OBJ->GetFileExtension($iCompanyIddoc[$i]['doc_file']);
                                            ...
                                            if ('is_image' === $file_ext) {
                                                $imgpath = $tsite_upload_compnay_doc.'/'.$_REQUEST['id'].'/'.$iCompanyIddoc[$i]['doc_file'];
                                                $resizeimgpath = $tconfig['tsite_url'].'resizeImg.php?src='.$imgpath.'&w=200';
                                                ?>
                                                <a href="<?php echo $tsite_upload_compnay_doc.'/'.$_REQUEST['id'].'/'.$iCompanyIddoc[$i]['doc_file']; ?>"
                                                   target="_blank">
                                            ...
                                            <?php } elseif ('is_pdf' === $file_ext) {
                                                ...
                                                <p>
                                                    <a href="<?php echo $tsite_upload_compnay_doc.'/'.$_REQUEST['id'].'/'.$iCompanyIddoc[$i]['doc_file']; ?>"
                                                       target="_blank">
                                                        <img src="<?php echo $resizeimgpath; ?>" style="cursor:pointer;"
                                                             alt="<?php echo $iCompanyIddoc[$i]['doc_name']; ?>"/>
                                                    </a>
                                                </p>
                                            <?php } else {
                                        ...
                                        <?php if ($userObj->hasPermission('manage-company-document')) { ?>
                                            <b>
                                                <button class="btn btn-info" data-toggle="modal" data-target="#uiModal"
                                                        id="custId"
                                                        onClick="setModel001('<?php echo $iCompanyIddoc[$i]['masterid']; ?>','<?php echo $iCompanyIddoc[$i]['ex_status']; ?>');">
                                                    <?php
            ...
            <?php if (0 !== $count_all2 && 'Yes' === $SET_DRIVER_OFFLINE_AS_DOC_EXPIRED) { ?>
                <div class="body-div">
                    <div class="form-group">
                        <div class="row">
                            <div class="col-sm-12">
                        ...
                        <div class="row company-document-action">
                            <?php for ($i = 0; $i < $count_all2; ++$i) { ?>
                                <div class="col-lg-3">
                                    <div class="panel panel-default upload-clicking">
                                        <div class="panel-heading"><?php echo $iCompanyIddoc2[$i]['doc_name']; ?>
                                        </div>
                                        <div class="panel-body" style="display: inline-block;">
                                            <?php if ('' !== $iCompanyIddoc2[$i]['req_file'] && file_exists('../webimages/upload/documents/company/'.$_REQUEST['id'].'/'.$db_userdoc[$i]['req_file'])) { ?>
                                                ...
                                                if ('is_image' === $file_ext) {
                                                    $imgpath1 = $tsite_upload_compnay_doc.'/'.$_REQUEST['id'].'/'.$iCompanyIddoc2[$i]['req_file'];
                                                    $resizeimgpath1 = $tconfig['tsite_url'].'resizeImg.php?src='.$imgpath1.'&w=200';
                                                    ?>
                                                    <a href="<?php echo $tsite_upload_compnay_doc.'/'.$_REQUEST['id'].'/'.$iCompanyIddoc2[$i]['req_file']; ?>"
                                                       target="_blank">
                                                ...
                                                <?php } else { ?>
                                                    <p>
                                                        <a href="<?php echo $tsite_upload_compnay_doc.'/'.$_REQUEST['id'].'/'.$iCompanyIddoc2[$i]['req_file']; ?>"
                                                           target="_blank"><?php echo $iCompanyIddoc2[$i]['doc_name']; ?></a>
                                                    </p>
                                                <?php } ?>
                                                <?php
                                            } else {
                                ...
                                ?>
                                            <?php if (!empty($iCompanyIddoc2[$i]['req_date'])) { ?>
                                                ...
                                                <input type="hidden" name="approvedIds[]" class="approvedIds"
                                                       value="<?php echo $iCompanyIddoc2[$i]['doc_id']; ?>">
                                            <?php } ?>
                                            <br/>
                                        </div>
                                    </div>
...
*/
    ...
    function setModel001(idVal, ex_status) {
        ...
        var ajaxData = {
            'URL': '<?php echo $tconfig['tsite_url_main_admin']; ?>company_document_fetch.php',
            'AJAX_DATA': 'rowid=' + idVal + '-' + <?php echo $_REQUEST['id']; ?>+'-' + ex_status + '&userType=<?php echo $_REQUEST['user_type']; ?>',
            'REQUEST_CACHE': false
        };
...
<script>
    $(document).on('click', '#approveDoc', function (event) {
        ...
        getDataFromAjaxCall(ajaxData, function (response) {
            if (response.action == "1") {
                window.location = 'company_document_action.php?success=4&id=<?php echo $_REQUEST['id']; ?>'+ '&userType=<?php echo $_REQUEST['user_type']; ?>;
            } else {
                console.log(response.result);
            }
        });
