...
/**
 * This Class is used to perform actions on trip_status_messages table.
 */
class TripStatusMsg
{
    ...
    public const TB_NAME = 'trip_status_messages';
    ...
    public function validateData(): void
    {
        if (empty($this->currentOperation)) {
            ...
            echo '1) TripStatusMsg::OP_INSERT <BR/>2) TripStatusMsg::OP_UPDATE <BR/>3) TripStatusMsg::OP_SELECT';
    ...
    private function retrieveSelectionFromArr($result_res)
    {
        ...
        for ($i = 0; $i < count($result_res); ++$i) {
            ...
            $result_res_item['iStatusId'] = $result_res_item['_id']['$oid'];
    ...
    private function executeProcessFromMongoDB()
    {
        ...
        if ($this->isSelectOp()) {
            ...
            if (!empty($this->currentOrderBy)) {
                if ('iStatusId' === $this->currentOrderBy) {
