$(document).ready(function(){
    ...
    $(document).on('click', '#add_config', function(){
		// confirm('WARNING: Are you sure you want to delete this connection?' + $('#app_context').val()+":::"+$('#new_conf_conn_string').val());
		
		
		if(isEmptyStr($('#new_conf_conn_name').val()) || isEmptyStr($('#new_conf_conn_string').val())){
			show_notification('Please enter both a connection name and connection string.', 'danger');
			return;
		}
		
		let connectionStrArr = $('#new_conf_conn_string').val().trim().split("/");
		let connection_db_str = connectionStrArr[connectionStrArr.length - 1];
		
		let app_context_val = $('#app_context').val();
		
		if(isEmptyStr(app_context_val)){
			show_notification('Context is required parameter. Please set it in config/app.json file.', 'danger');
			return;
		}
		
		if(isEmptyStr(connection_db_str)){
			show_notification('Please define DB in your connection.', 'danger');
			return;
		}else if(!app_context_val.endsWith(connection_db_str)){
			show_notification('Connection to DB "'+connection_db_str+'" is not allowed.', 'danger');
			return;
		}
		
		/* confirm('WARNING: Are you sure you want to delete this connection?' + $('#app_context').val()+":::"+connectionStrArr[connectionStrArr.length - 1]); */
		...
		// show_notification('Please enter both a connection name and connection string::'+connectionStrArr[connectionStrArr.length - 1] + "::Context::"+$('#app_context').val(), 'danger');
		
		// check DB name with existing context DB
