...
if ($type == "displayWayBill") {
    ...
    if (count($tripData) > 0) {
        ...
        $vCurrencyDriver = $driver_detail[0]['vCurrencyDriver'];
        ...
        $UserAddressArr = FetchMemberAddressData($orders[0]['iUserId'], "Passenger", $orders[0]['iUserAddressId']);

        $UserAdress =mb_convert_case($passengername, MB_CASE_TITLE, 'UTF-8'). "\n" . $UserAddressArr['UserAddress'];
     //   $UserAdress = ucfirst($passengername) . "\n" . $UserAddressArr['UserAddress'];
        ...
        $order_fDeliveryCharge = formateNumAsPerCurrency($fDeliveryCharge,$vCurrencyDriver);
        ...
        $tripArr['tSaddress'] = mb_convert_case($vCompany, MB_CASE_TITLE, 'UTF-8'). "\n" . $vRestuarantLocation;
