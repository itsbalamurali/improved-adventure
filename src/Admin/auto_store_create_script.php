...
$cusineUnknownFields = ['SI', 'TA', 'VI', 'TL', 'KM', 'BN'];
$foodMenuNewLang = ['SI', 'TA', 'VI', 'TL', 'KM', 'BN'];
$menuItemNewLang = ['SI', 'TA', 'VI', 'TL', 'KM', 'BN'];
$itemPriceArr = ['199.00', '249.00'];
if (isset($_POST['submit'])) {
    ...
    $catNameArr = $cuisineNameArr = $foodMenuArr = $foodMenuDescArr = $menuItemsArr = $menuItemDescArr = [];
    if ($count_all > 0) {
        for ($i = 0; $i < $count_all; ++$i) {
            ...
            $vTitle_store[] = $vValue;
            if (!in_array($db_master[$i]['vCode'], $cusineUnknownFields, true)) {
                $cuisineNameArr[] = $cusineValue;
            }
            if (!in_array($db_master[$i]['vCode'], $foodMenuNewLang, true)) {
                $foodMenuArr[] = $foodValue;
                $foodMenuDescArr[] = $foodDesc;
            }
            if (!in_array($db_master[$i]['vCode'], $menuItemNewLang, true)) {
    ...
    if (isset($_POST['servicecat'])) {
        ...
        if ('' !== $serviceCategory) {
            ...
            if ($cat_id > 0) {
                ...
                // Cuisine - All
                $cuisine_sql = $q.' `'.CUISINE."` SET `cuisineName_EN` = 'All', `cuisineName`='All',`iServiceId` = '".$cat_id."',`eStatus`= 'Active', `iDisplayOrder`= '0', `eDefault` = 'Yes'";
                $obj->sql_query($cuisine_sql);
                // Cuisine - All End
                ...
                if ($cuisine_id > 0) {
                    for ($c = 0; $c < $insertStoreRecord; ++$c) {
                        ...
                        $time_val = time();
                        $sample_filename_rand = random_int(11_111, 99_999);
                        ...
                        if ($store_id > 0) {
                            ...
                            for ($f = 0; $f < $insertCategoryRecord; ++$f) {
                                ...
                                $food_sql = $q.' `'.FOOD_MENU."` SET {$foodNames} {$foodDesc}  `iCompanyId`='".$store_id."',`iServiceId`='".$cat_id."',`eStatus`= 'Active',`iDisplayOrder`='".$fCount."'";
                ...
                <div class="inner">
                    ...
                    <div class="body-div">
                        <div class="form-group">
                            <?php if (isset($success) && 1 === $success) { ?>
