...
if ($THEME_OBJ->isXThemeActive() == 'Yes') {
    ...
    if(($THEME_OBJ->isRideCXThemeActive() == 'Yes' || $THEME_OBJ->isRideDeliveryXThemeActive() == 'Yes' || $THEME_OBJ->isRideCXv2ThemeActive() == 'Yes') && !empty($_REQUEST['type']) && ($_REQUEST['type']=='restaurant' || $_REQUEST['type']=='store')) {
        header("location: sign-up?type=user");
        exit;
    }
...
if($_REQUEST["vRefCode"] != ''){
    $_SESSION["vRefCode"] = $_REQUEST["vRefCode"];
}else if($_SESSION["vRefCode"] != ''){
    $_SESSION["vRefCode"] = $_SESSION["vRefCode"];
}else{
    $_SESSION["vRefCode"] = "";
}
...
<?php if ($REFERRAL_SCHEME_ENABLE == 'Yes') { ?>
                                    <span class="newrow"><strong id="refercodeCheck">
                                            ...
                                            <input id="vRefCode" type="text" name="vRefCode" placeholder="<?= $langage_lbl['LBL_SIGNUP_REFERAL_CODE']; ?>" class="create-account-input create-account-input1 vRefCode_verify" value="<?php echo $_SESSION["vRefCode"]; ?>" onBlur="validate_refercode(this.value)"/></strong></span> 
                                <?php } ?>
...
<?php include_once("recaptcha.php"); ?>
                                ...
                                <span class="newrow">
                                    <strong class="captcha-signup1">
                                        <abbr><?= $langage_lbl['LBL_SIGNUP_Agree_to']; ?> <a href="terms-condition" target="_blank"><?= $langage_lbl['LBL_SIGN_UP_TERMS_AND_CONDITION']; ?></a>
                                                    ...
                                                    var request = $.ajax({
                                                        ...
                                                        success: function (data)
                                                        {
                                                            if (data == 0) {
                                                                ...
                                                                $(".vRefCode_verify").removeClass('required-active');
                                                                $('#referCheck').remove();
                                                            }
                                                        }
                                                    });
