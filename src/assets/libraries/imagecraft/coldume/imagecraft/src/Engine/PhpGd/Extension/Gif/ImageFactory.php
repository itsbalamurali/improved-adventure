...
class ImageFactory
{
     ...
     * @param LayerInterface[] $layers
     */
    protected function initContents(array $layers): void
    {
    ...
    protected function initQuality(BackgroundLayerInterface $layer): void
    {
        ...
        if ((250_000 > $width * $height) && 2_400_000 > $pixels) {
            $quality = true;
        } else {
            $quality = false;
        }
     ...
     * @param LayerInterface[] $layers
     */
    protected function initCacheResource(array $layers): void
    {
        if (1 === \count($layers)) {
            return;
        }
    ...
    protected function initImageAwareLayerResource(ImageAwareLayerInterface $layer): void
    {
    ...
    protected function initTextLayerResource(TextLayerInterface $layer): void
    {
    ...
    protected function initFrameResource(BackgroundLayerInterface $layer): void
    {
        $extracted = $layer->get('gif.extracted');
        $this->builder
    ...
    protected function coalesceFrameResource(BackgroundLayerInterface $layer): void
    {
        $extracted = $layer->get('gif.extracted');
    ...
    protected function renderFrameResource(BackgroundLayerInterface $layer): void
    {
        if ($layer->has('image.resize.width')) {
            $resource = $this->rh->getResizedGdResource(
    ...
    protected function mergeFrameResource(BackgroundLayerInterface $layer): void
    {
    ...
    protected function initFrameContents(BackgroundLayerInterface $layer): void
    {
        $extracted = $layer->get('gif.extracted');
        $quality = $layer->get('gif.quality');
        $resource = $layer->get('gif.frame_resource');
