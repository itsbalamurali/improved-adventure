...
$ParameterUrl = 'eType='.$_REQUEST['eType'].'&selectlang='.$vCodeLang;

// delete record
// echo $hdn_del_id;die;
if ('' !== $hdn_del_id) {
    if (SITE_TYPE !== 'Demo') {
        $data_q = 'SELECT Max(iDisplayOrder) AS iDisplayOrder FROM `'.$tbl_name."` WHERE 1  {$whereserviceId} ";

        $data_rec = $obj->MySQLSelect($data_q);

        $order = $data_rec[0]['iDisplayOrder'] ?? 0;

        $data_logo = $obj->MySQLSelect('SELECT iDisplayOrder FROM '.$tbl_name." WHERE iUniqueId = '".$hdn_del_id."' {$whereserviceId} ");

        if (count($data_logo) > 0) {
            $iDisplayOrder = $data_logo[0]['iDisplayOrder'] ?? '';
            $obj->sql_query('DELETE FROM `'.$tbl_name."` WHERE iUniqueId = '".$hdn_del_id."' {$whereserviceId}");

            if ($iDisplayOrder < $order) {
                for ($i = $iDisplayOrder + 1; $i <= $order; ++$i) {
                    $obj->sql_query('UPDATE '.$tbl_name.' SET iDisplayOrder = '.($i - 1).' WHERE iDisplayOrder = '.$i.$whereserviceId);
                }
            }
        }

        header('Location:bsr_banner.php?'.$ParameterUrl);
...
$db_data1 = $obj->MySQLSelect('SELECT * FROM '.$tbl_name." WHERE 1 {$whereserviceId}  AND eFor = 'General' ORDER BY iDisplayOrder");

// $db_data2 = $obj->MySQLSelect("SELECT * FROM " . $tbl_name . " WHERE 1 $whereserviceId AND vCode != '" . $default_lang . "' AND eFor = 'General' ORDER BY vCode,iDisplayOrder");
                ...
                <div class="inner">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12">
                            ...
                            <a href="bsr_banner_action.php?eType=<?php echo $_REQUEST['eType']; ?>">
    ...
    for ($i = 0; $i < $count_all; ++$i) {
        ...
        ?>
                                                    ...
                                                    <tr class="gradeA">
                                                        ...
                                                        <td align="center" width="10%"><?php echo $vCode; ?></td>

                                                        <td width="10%" align="center"><?php echo $iDisplayOrder; ?> </td>

                                                        <!-- <?php if ($userObj->hasPermission($updateStatus)) { ?>

                                                        <td width="10%" align="center">

                                                            <a href="bsr_banner.php?eType=<?php echo $_REQUEST['eType']; ?>&iUniqueId=<?php echo $iUniqueId; ?>&vCode=<?php echo $vCode; ?>&selectlang=<?php echo $vCode; ?>&status=<?php echo ('Active' === $eStatus) ? 'Inactive' : 'Active'; ?>">
                                                        ...
                                                        <td width="10%" align="center">
                                                            ...
                                                            <a href="bsr_banner_action.php?eType=<?php echo $_REQUEST['eType']; ?>&id=<?php echo $iUniqueId; ?>&vCode=<?php echo $vCode; ?>">
                                                        ...
                                                        <td width="10%" align="center">
                                                            ...
                                                            <form name="delete_form" id="delete_form" method="post" action="" onsubmit="return confirm_delete()" class="margin0">
                                                                ...
                                                                <input type="hidden" name="iMasterServiceCategoryId" id="iMasterServiceCategoryId" value="<?php echo $iMasterServiceCategoryId; ?>">
                                                                ...
                                                                <div class="social show-moreOptions openPops_<?php echo $iUniqueId; ?>">
                                                                    <ul>
                                                                        ...
                                                                        <li class="entypo-twitter" data-network="twitter">
                                                                            <a href="bsr_banner_action.php?eType=<?php echo $_REQUEST['eType']; ?>&id=<?php echo $iUniqueId; ?>&vCode=<?php echo $vCode; ?>" data-toggle="tooltip" title="Edit">
                                                                            <img src="img/edit-icon.png" alt="Edit">
                                                                            </a></li>
                                                                        <?php }  ?>
                                                                        <?php if ($userObj->hasPermission($updateStatus)) { ?>
                                                                            <li class="entypo-facebook" data-network="facebook">
                                                                                <a href="javascript:void(0);" onClick='window.location.href="bsr_banner.php?eType=<?php echo $_REQUEST['eType']; ?>&iUniqueId=<?php echo $iUniqueId; ?>&status=Active<?php echo ('' !== $eBuyAnyService) ? '&'.$eBuyAnyService : (('' !== $eForDelivery) ? '&'.$eForDelivery : ''); ?>&langSearch=<?php echo $vCode; ?>&vCode=<?php echo $vCode; ?><?php echo (in_array($eType, ['NearBy', 'Runner', 'Genie'], true)) ? '&eType='.$eType : ''; ?>"' data-toggle="tooltip" title="Activate">
                                                                                    <img src="img/active-icon.png" alt="<?php echo $eStatus; ?>">
                                                                                </a>
                                                                            </li>
                                                                            <li class="entypo-gplus" data-network="gplus">

                                                                                <a href="javascript:void(0);" onClick='window.location.href="bsr_banner.php?eType=<?php echo $_REQUEST['eType']; ?>&iUniqueId=<?php echo $iUniqueId; ?>&status=Inactive<?php echo ('' !== $eBuyAnyService) ? '&'.$eBuyAnyService : (('' !== $eForDelivery) ? '&'.$eForDelivery : ''); ?>&langSearch=<?php echo $vCode; ?>&vCode=<?php echo $vCode; ?><?php echo (in_array($eType, ['NearBy', 'Runner', 'Genie'], true)) ? '&eType='.$eType : ''; ?>"' data-toggle="tooltip" title="Deactivate">
                                                                        ...
                                                                        <?php } ?>
                                                                        <?php if ($userObj->hasPermission($delete)) {  ?>
                                                                            <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onClick="confirm_delete('<?php echo $iUniqueId; ?>','<?php echo $vCode; ?>','<?php echo $iMasterServiceCategoryId; ?>');" data-toggle="tooltip"  title="Delete">
...
?>
        ...
        <form name="pageForm" id="pageForm" action="" method="post" >
            ...
            <input type="hidden" name="eType" id="eType" value="<?php echo $eMasterType; ?>" >
        </form>
        ...
        <script>
            ...
            function confirm_delete(iUniqueId,vCode,iMasterServiceCategoryId) {

                var confirm_ans = confirm("Are You sure You want to Delete Banner?");

                if (confirm_ans == true) {
                    window.location.href = 'bsr_banner.php?eType=<?php echo $_REQUEST['eType']; ?>&hdn_del_id='+iUniqueId+'&vCode='+vCode+'&iMasterServiceCategoryId='+iMasterServiceCategoryId;
                }
            }
        </script>
    </body>
