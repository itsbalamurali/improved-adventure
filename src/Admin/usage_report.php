...
// echo "<br>";
// echo date('Y-m-d h:i:s A', strtotime("+5 hours +30 minutes"));
$utc = date('M d Y');
$time = strtotime($utc);
...
$actualURL = (isset($_SERVER['HTTPS']) && 'on' === $_SERVER['HTTPS'] ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].$oidURL.$sidURL.$authacc_oid_URL;
$actualURLWithDuration = (isset($_SERVER['HTTPS']) && 'on' === $_SERVER['HTTPS'] ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].$oidURL.$sidURL.$durationURL;
$actualURLWithRangeDate = (isset($_SERVER['HTTPS']) && 'on' === $_SERVER['HTTPS'] ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].$oidURL.$sidURL.$add_date_to_url;
if ('' !== $duration) {
...
if ('' === $authacc_oid) {
    $searchQueryForDropDown1['vServiceId'] = (int) $sid;
    $searchQueryForDropDown1['eStatus'] = 'Active';
    $data_from_auth_accounts_for_dropdown1 = $obj->fetchAllRecordsFromMongoDBWithDBName($DbName, $TableName_Auth_account, $searchQueryForDropDown1);
    $authacc_oid = $data_from_auth_accounts_for_dropdown1[0]['_id']['$oid'];
}

if ('' !== $oid) {
    $activeServiceAry = [];
    $searchQueryServiceName['_id'] = new ObjectID($oid);
...
if ('' !== $today && '' !== $fromDate) {
    if ($today !== $fromDate) {
        $dates['$lte'] = new UTCDateTime(strtotime($today.'00:00:00') * 1_000);
        $dates['$gte'] = new UTCDateTime(strtotime($fromDate.'00:00:00') * 1_000);
        $searchQueryReport['vUsageDate'] = $dates;
    } else {
        $searchQueryReport['vUsageDate'] = new UTCDateTime(strtotime($today.'00:00:00') * 1_000);
    }
} else {
    $searchQueryReport['vUsageDate'] = new UTCDateTime(strtotime($dateInLocal.'00:00:00') * 1_000);
}
$allDataDateWise = $obj->fetchAllRecordsFromMongoDBWithDBName($DbName, $TableName, $searchQueryReport);
if ('' !== $sid) {
    $searchQuery['vServiceId'] = (int) $sid;
}
if ('' !== $authacc_oid) {
    $searchQuery['_id'] = new ObjectID($authacc_oid);
}
if ('' !== $sid || '' !== $authacc_oid) {
    $data_from_auth_accounts = $obj->fetchAllRecordsFromMongoDBWithDBName($DbName, $TableName_Auth_account, $searchQuery);
}
if ('' !== $sid) {
    $searchQueryForDropDown['vServiceId'] = (int) $sid;
...
foreach ($machedRecords as $value) {
    ...
    $ReverseGeoCode += (int) $value['ReverseGeoCode'];
    $AutoComplete += (int) $value['AutoComplete'];
    $PlaceDetails += (int) $value['PlaceDetails'];
}
$PlaceAPI = (int) $AutoComplete + (int) $PlaceDetails;
...
?> |  <?php echo $langage_lbl_admin['LBL_MAP_API_USAGE_REPORT_ADMIN']; ?></title>
        ...
        <!-- commented NM on 18-9-20 no required to show graph -->
    ...
    <script type="text/javascript">
        $(document).ready(function(){
                ...
                $( "#dateRangeForm" ).submit(function( event ) {
                    ...
                    window.location.href = '<?php echo $actualURLWithDuration.$authacc_oid_URL; ?>'+'&startdate='+startdate+'&enddate='+enddate+'';
...
if ('usage_report.php' === $current_filename) { ?>
                                ...
                                <div style="float: right;  display: flex;">
                                    <a target="_blank" class="btn btn-success btn-ok " type="button" value="View Account" href="map_api_mongo_auth_places.php?id=<?php echo ($sid) ?: $vServiceId; ?>"> <?php echo 'View Account'; ?> </a>
                                    <?php if (1 === $less) { ?>
                    ...
                    <div class="table-list" style="padding:23px !important">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="admin-nir-export">
                                    <div class="changeStatus col-lg-6 option-box-left">
                                        <?php if (count($data_from_auth_accounts_for_dropdown) > 1) { ?>
                                        <span for="#ChangeService"><strong>Accounts: </strong></span>
                                        <select name="ChangeService" id="ChangeService" class="form-control" onChange="ChangeService(this.value);" style="width:auto; display: inline-block; margin-left: 10px;">
                                        <?php foreach ($data_from_auth_accounts_for_dropdown as $key => $authAccounts) {
                                            if (('' === $authacc_oid) && (0 === $authacc_oid) && ('0' === $key)) {
                                                $active = ' selected';
                                            } elseif ($authacc_oid === $authAccounts['_id']['$oid']) {
                                                $active = ' selected';
                                            } else {
                                                $active = ' ';
                                            }
                                    ...
                                    <div class="usage_report_button_list" >
                                        <a href="<?php echo $actualURL; ?>" class="usage_report_button <?php echo ('' === $duration) ? ' active' : ''; ?>" >Today</a>
                                        <a href="<?php echo $actualURL.'&duration=2'; ?>" class="usage_report_button <?php echo ('2' === $duration) ? ' active' : ''; ?>">2 Days</a>
                                        <a href="<?php echo $actualURL.'&duration=7'; ?>" class="usage_report_button <?php echo ('7' === $duration) ? ' active' : ''; ?>">7 Days</a>
                                        <a href="<?php echo $actualURL.'&duration=15'; ?>" class="usage_report_button <?php echo ('15' === $duration) ? ' active' : ''; ?>">15 Days</a>
                                        <a href="<?php echo $actualURL.'&duration=30'; ?>" class="usage_report_button <?php echo ('30' === $duration) ? ' active' : ''; ?>">30 Days</a>
                                        <a href="#" data-toggle="modal" class="usage_report_button <?php echo ('range' === $duration) ? ' active' : ''; ?>" data-target="#datepicker-modal ">Select Date Range </a>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-lg-6">
                                        <?php if ($userObj->hasPermission('view-map-api-service-account')) { ?>
                                        <div class="panel panel-primary bg-gray-light" >
                                            ...
                                            <div class="row padding_005">
                                                ...
                                                if (in_array('Geocoding', $activeServiceAry, true) || (('' !== $ReverseGeoCode) && (0 !== $ReverseGeoCode))) { ?>
                                                <div class="col-lg-6">
                                                    <div class="info-box bg-aqua">
                                                        ...
                                                        <div class="info-box-content info-box-content-space">
                                                            ...
                                                            <span class="info-box-number"><?php echo number_format($ReverseGeoCode); ?></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <?php
                                                } ?>
                                                <?php if (in_array('Direction', $activeServiceAry, true) || ('' !== $Direction && '0' !== $Direction)) { ?>
                                                <div class="col-lg-6">
                                                    <div class="info-box bg-yellow">
                                                        ...
                                                        <div class="info-box-content info-box-content-space">
                                                            ...
                                                            <span class="info-box-number"><?php echo ('' !== $Direction || '0' !== $Direction) ? number_format($Direction) : '0'; ?></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <?php
                                                ...
                                                ?>
                                                <?php if (in_array('AutoComplete', $activeServiceAry, true) || ('' !== $PlaceAPI && 0 !== $PlaceAPI)) { ?>
                                                <div class="col-lg-6">
                                                    <div class="info-box bg-red">
                                                        ...
                                                        <div class="info-box-content info-box-content-space">
                                                            ...
                                                            <span class="info-box-number"><?php echo number_format($PlaceAPI); ?></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <?php
                    ...
                    <div class="modal-body" style="max-height: 450px;overflow: auto;">
                        ...
                        <div>
                            <form name="_import_map_api_settings" id="_import_map_api_settings" enctype="multipart/form-data" method="POST" onsubmit="return confirm('Are you sure to remove your current map api setting data?');">
                                <div style="color:#1fbad6;"><b>Note:</b> Please note that all map api setting data will be removed.</div>
                                <br>
                        ...
                        <form id="dateRangeForm" name="dateRangeForm" method="get" autocomplete="off">
                            <div class="form-group">
                                <div class='input-group date' id='startdate' >
                                    <input  name="edate" placeholder='Select Start Date' type='text' value="<?php echo ('' !== $startDate) ? $startDate : ''; ?>" title="Select Start Date" autocomplete="off" class="form-control" required  readonly/>
                            ...
                            <div class="form-group" >
                                <div class='input-group date'  id='enddate' >
                                    <input name="sdate" value="<?php echo ('' !== $endDate) ? $endDate : ''; ?>" title="Select End Date" placeholder='Select End Date' type='text' autocomplete="off" class="form-control" required readonly/>
                                    <span class="input-group-addon">
                                    <span class="glyphicon glyphicon-calendar" ></span>
                                    </span>
                                </div>
                                ...
                                <?php if ('' !== $authacc_oid) { ?>
                                <input type='hidden' name="auth_oid" value="<?php echo $authacc_oid;
                                    ?>" />
                                <?php
                                } ?>
                            </div>
            ...
            <div class="modal-dialog" >
                <div class="modal-content">
                    <div class="modal-header">
                        <h4>
                            <i style="margin:2px 5px 0 2px;"><img src="images/icon/driver-icon.png" alt=""></i>
                            <?php echo $langage_lbl_admin['LBL_MAP_API_USAGE_REPORT_ADMIN']; ?> Details
                            <button type="button" class="close" data-dismiss="modal">x</button>
                        </h4>
                    </div>
                    <div class="modal-body" style="max-height: 450px;overflow: auto;">
        ...
        <script>
            ...
            getData = function() {
             ...
             return [
               ...
               <?php if (in_array('Geocoding', $activeServiceAry, true) || ('' !== $ReverseGeoCode && '0' !== $ReverseGeoCode)) { ?>

                   { "company": "Geocoding : <?php echo (int) $ReverseGeoCode; ?>",  "value": <?php echo ('' !== $ReverseGeoCode || '0' !== $ReverseGeoCode) ? ((int) $ReverseGeoCode) : '0'; ?>},

               <?php
               }
if (in_array('Direction', $activeServiceAry, true) || ('' !== $Direction && '0' !== $Direction)) { ?>

                   { "company": "Direction : <?php echo (int) $Direction; ?>", "value": <?php echo ('' !== $Direction || '0' !== $Direction) ? (int) $Direction : '0'; ?> },

               <?php
}
if (in_array('AutoComplete', $activeServiceAry, true) || in_array('PlaceDetails', $activeServiceAry, true) || ('' !== $PlaceAPI && '0' !== $PlaceAPI)) { ?>

                   { "company": "AutoComplete : <?php echo (int) $PlaceAPI; ?>", "value": <?php echo ('' !== $PlaceAPI || '0' !== $PlaceAPI) ? (int) $PlaceAPI : '0'; ?> }
        ...
        <script>
            function ChangeService(oid){
               ...
               window.location.href = '<?php echo $actualURLWithRangeDate; ?>'+'&auth_oid='+oid;
