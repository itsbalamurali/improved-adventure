...
async.forEachOf(connection_list, function (value, key, callback){
    ...
    function (err){
		...
		if(isEmptyStr(app_context)){
			console.log('Context is required parameter. Please set it in config/app.json file.');
			process.exit();
			return;
		}
		...
		if(app_host == "localhost"){
			app.listen(app_port, function (){
				...
				console.log('adminMongo listening on port: ' + app_port + ' and databse: ' + app_context);
