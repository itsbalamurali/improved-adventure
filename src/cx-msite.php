...
if($msite=='Yes') {
    $_SESSION['is_msite'] = 'Yes';
} else {
    $_SESSION['is_msite'] = 'No';
    if ($_REQUEST['userType1'] == 'admin' && empty($_SESSION['sess_iAdminUserId'])) {
        $_SESSION['login_redirect_url'] = $tconfig["tsite_url_main_admin"];
        header("location: ".$tconfig["tsite_url_main_admin"]);exit;
    }
