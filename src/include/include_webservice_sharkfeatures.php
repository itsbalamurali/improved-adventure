...
function getAdvertisementBanners($iUserId, $eMemberType) {
    ...
    $getBanners = $obj->MySQLSelect("SELECT iAdvertBannerId,tRedirectUrl,eValidityType,eImpression,dStartDate,dExpiryDate,iImpression,vBannerImage, (SELECT count(iBannerImpLog) as TOTAL FROM banner_impression WHERE iAdvertBannerId = advertise_banners.iAdvertBannerId) as usedCount FROM advertise_banners WHERE eStatus='Active' ORDER BY iDispOrder ASC");
    ...
    if ($iUserId > 0) {
        ...
        for ($r = 0; $r < count($getBanners); $r++) {
            ...
            $usedCount = $getBanners[$r]['usedCount'];
...
function getAdvertisementBannersAsPerDevice($iMemberId, $UserType) {
    global $obj, $demo_site_msg, $tconfig, $_REQUEST, $ENABLE_RIDER_ADVERTISEMENT_BANNER, $ENABLE_DRIVER_ADVERTISEMENT_BANNER, $ENABLE_RESTAURANTS_ADVERTISEMENT_BANNER, $ADVERTISEMENT_TYPE;
    ...
    if ($ADVERTISEMENT_TYPE != "Disable") {
        ...
        if (isset($adBannerData['iAdvertBannerId']) && $adBannerData['iAdvertBannerId'] > 0) {
            ...
            if (file_exists($pathOfAdvImage_chk)) {
                $imagedata = getimagesize($pathOfAdvImage_chk);
...
function getNewsNotification($page, $iMemberId, $eMemberType, $eType) {
    ...
    $defaultlangCode = $LANG_OBJ->FetchDefaultLangData("vCode");
    ...
    if ($eType == "News" || $eType == "All") {
        ...
        $getNewData = $obj->MySQLSelect("SELECT JSON_UNQUOTE(JSON_EXTRACT(vTitle, '$.vTitle_" . $langCode . "')) as vTitle,JSON_UNQUOTE(JSON_EXTRACT(tDescription, '$.tDescription_" . $langCode . "')) as tDescription,tPublishdate,vNewfeedImage AS vImage FROM newsfeed WHERE (eUserType='" . $eMemberType . "' OR eUserType='" . $eMemberTypeall . "') AND eStatus='Active' AND vTitle!='' AND eType='News' ORDER BY tPublishdate DESC");
    ...
    if ($eType == "Notification" || $eType == "All") {
        ...
        // echo "SELECT iPushnotificationId,tMessage,dDateTime FROM pushnotification_log WHERE eUserType='" . $eMemberType . "' AND iUserId='" . $iMemberId . "' AND tMessage != '' ORDER BY iPushnotificationId DESC $limit"; exit;
    ...
    if (count($getNewData) > 0 && count($notificationarr) > 0) {
        ...
        if (count($newArr) > 0) {
            $per_page = 10;
            $TotalPages = ceil(count($newArr) / $per_page);
            $start_limit = ($page - 1) * $per_page;
            if ($TotalPages > $page) {
                $returnArr['NextPage'] = ($page + 1);
            }

            $end_limit = $start_limit + $per_page;
            $newArr = array_slice($newArr, $start_limit, $end_limit);
            $returnArr['Action'] = "1";
...
function update_driver_insurance_status($iDriverId, $eFor, $details_arr = array(), $eFromType = "", $vStatus = "", $allow_blank_latlong = "No") {
    global $obj, $ENABLE_INSURANCE_IDLE_REPORT, $ENABLE_INSURANCE_ACCEPT_REPORT, $ENABLE_INSURANCE_TRIP_REPORT;
    ...
    if ($action == "Add") {
        if ($Allow_Insert_record == "Yes") {
            ...
            if (isset($details_arr['LatLngArr']['vLatitude']) && $details_arr['LatLngArr']['vLatitude'] != "") {
                ...
                $Data_update['tStartLocation'] = isset($details_arr['LatLngArr']['vLocation']) ? $details_arr['LatLngArr']['vLocation'] : "";
            }
        ...
        if (isset($data_insurance[0]['iInsuranceReportId']) && $data_insurance[0]['iInsuranceReportId'] > 0 && ($eFromType == "GenerateTrip" || $eFromType == "StartTrip")) {
            ...
            $Data_update_old['tEndLocation'] = isset($details_arr['LatLngArr']['vLocation']) ? $details_arr['LatLngArr']['vLocation'] : "";
        ...
        if (((isset($details_arr['LatLngArr']['vLatitude']) && $details_arr['LatLngArr']['vLatitude'] != "") || ($allow_blank_latlong == "Yes" && $eFromType == "cancelTrip"))) {
            if (isset($data_insurance[0]['iInsuranceReportId']) && $data_insurance[0]['iInsuranceReportId'] > 0) {
                ...
                $Data_update['tEndLocation'] = isset($details_arr['LatLngArr']['vLocation']) ? $details_arr['LatLngArr']['vLocation'] : "";
...
function BookForSomeOneElse() {
    global $obj, $Data_update_passenger, $Data, $type;
...
function UpdateOrganizationTripOutstandingAmount($iTripId, $ePaidByPassenger = "No", $ePaidToDriver = "No") {
    global $obj;
    $sql = "SELECT iUserId,iDriverId,fCancellationFare,fWalletDebit,vTripPaymentMode,vRideNo,iVehicleTypeId,iOrganizationId,ePaymentBy,iFare,fTax1,fTax2 FROM trips WHERE iTripId='" . $iTripId . "'";
...
function reDefineGenerateTripParam() {
    global $iCabRequestId, $iCabBookingId, $Data_trips, $POOL_ENABLE, $driver_id, $data_booking, $check_row, $obj, $Data_passenger_detail, $VehicleData, $DriverVehicleMake, $DriverVehicleModel, $Data_vehicle, $iTripId, $SITE_ISD_CODE, $PACKAGE_TYPE, $vTripPaymentMode, $vLangCode, $isdCode, $BOOK_FOR_ELSE_ENABLE, $Data_update_driver;
...
function finalizeProcessEndTrip() {
    global $obj, $trip_start_data_arr, $tripId, $userId, $Data_update_trips;
...
function GetPassengerOutstandingAmountShark($iUserId, $NOT_IN = "", $GET_DATA = "No") {
    global $obj, $_REQUEST, $data_trips, $iOrganizationId, $ePaymentBy, $type, $SYSTEM_PAYMENT_FLOW, $ePayWallet;
    ...
    if(isset($data_trips[0]['iOrganizationId']) && $data_trips[0]['iOrganizationId'] > 0) {
    ...
    if(isset($data_trips[0]['ePaymentBy']) && strtolower($data_trips[0]['ePaymentBy']) == "organization") {
    ...
    $fieldSql = " SUM(fPendingAmount) as fPendingAmount ";
    if(strtoupper($GET_DATA) == "YES") {
        $fieldSql = " GROUP_CONCAT(iTripOutstandId) as iTripOutstandIds ";
    }
    $outStandingSql = "";
    ...
    if ($ePaymentBy == "Passenger" || $ePaymentBy == "Sender") {
        $sql = "SELECT $fieldSql FROM trip_outstanding_amount WHERE iUserId='" . $iUserId . "' AND iUserId > 0 AND ePaidByPassenger = 'No' AND ePaymentBy = 'Passenger' $outStandingSql";
        ...
        $sql = "SELECT $fieldSql FROM trip_outstanding_amount WHERE iUserId = '" . $iUserId . "' AND iUserId > 0 AND iOrganizationId    ='" . $iOrganizationId . "' AND ePaidByOrganization = 'No' AND ePaymentBy = 'Organization' AND eBillGenerated = 'No' $outStandingSql";
    ...
    if(!empty($NOT_IN)) {
        $sql .= " AND iTripOutstandId NOT IN ($NOT_IN) ";
    }

    $tripoutstandingdata = $obj->MySQLSelect($sql);

    if(strtoupper($GET_DATA) == "YES") {
        return $tripoutstandingdata[0]['iTripOutstandIds'];
    }
...
function getDriverPoolTrips($driverId) {
    ...
    global $obj, $GOOGLE_SEVER_GCM_API_KEY, $POOL_ENABLE,$vSystemDefaultLangCode,$Data_ALL_langArr, $LANG_OBJ;
...
function fetchtripstatustimeintervalForPool() {
    global $obj, $FETCH_TRIP_STATUS_TIME_INTERVAL_POOL, $Data, $POOL_ENABLE, $_REQUEST;
...
function fetchtripstatustimeMAXintervalForPool() {
    global $obj, $FETCH_TRIP_STATUS_TIME_INTERVAL_POOL;
...
function getPoolDriverList($poolDriverArr, $Check_Driver_UFX, $isFromHotelPanel, $vLatitude, $vLongitude, $param, $sourceLat, $str_date, $sourceLon, $destLat, $destLon) {
    global $obj, $FEMALE_RIDE_REQ_ENABLE, $RESTRICTION_KM_NEAREST_TAXI_POOL, $RESTRICTION_KM_NEAREST_DESTINATION_POOL, $POOL_ENABLE;
    ...
    if (!empty($Data)) {   //added by SP on 22-7-2019 bc when no data the error in vTripStatusnot found in manual booking
        for ($r = 0; $r < count($Data); $r++) {
            ...
            if (isset($checkPoolTrip[0]['ePoolRide']) && $checkPoolTrip[0]['ePoolRide'] == "Yes") {
                //$poolDriverArr[] = $Data[$r]; //Commneted By HJ On 29-09-2020 For Get Driver List In Sequence Tested By GP in main copy
                $newPoolDriverArr[] = $Data[$r]; //Added By HJ On 29-09-2020 For Get Driver List In Sequence Tested By GP in main copy
            }
        }
    }
    $poolDriverArr = array_merge($newPoolDriverArr,$poolDriverArr); //Added By HJ On 29-09-2020 For Get Driver List In Sequence Tested By GP in main copy
    return $poolDriverArr;
}
...
function poolVariableGetForSendRequest() {
    global $obj, $_REQUEST, $POOL_ENABLE, $selectedCarTypeID, $final_message, $Data_update_passenger, $driver_id_auto, $ePoolStatus,$vehicleTypeDataArr;
    if ($POOL_ENABLE != "Yes") {
        return;
    }
...
function reGenerateRequestQueryForDriver() {
    global $ePoolStatus, $sql_driver_status_chk, $POOL_ENABLE, $str_date, $eType, $MODULES_OBJ;

    if ($ePoolStatus == "Yes" && $POOL_ENABLE == "Yes") {
        $sql_driver_status_chk .= " AND tLocationUpdateDate > '$str_date' AND ((vAvailability='Not Available' AND (vTripStatus='Active' || vTripStatus='On Going Trip' || vTripStatus='Arrived')) OR vAvailability='Available')";
    } else {
        if($MODULES_OBJ->isSendRequestToDriverBeforFinishTripModule() && $eType == "Ride")
        {
            $sql_driver_status_chk .= "AND ((tLocationUpdateDate > '$str_date' AND vAvailability='Available' )|| (vAvailability = 'Not Available' AND vTripStatus = 'On Going Trip') || (vTripStatus = 'Not Active' AND vAvailability='Available'))";
        
        }else{
            $sql_driver_status_chk .= " AND tLocationUpdateDate > '$str_date' AND vAvailability='Available'";
        }
    }
...
function configureTripForArriveStatus() {
    global $obj, $result, $message_arr, $_REQUEST, $languageLabelsArr;
...
if ($type == "UpdateUserOrganizationProfile") {
    ...
    if ($id > 0) {
        ...
        if ($Action == "Add") {
            ...
            $mail = $COMM_MEDIA_OBJ->SendMailToMember('USER_REGISTRATION_ORGANIZATION', $maildata);
        }
        ## Send Email To Organization ##
    } else {
        $returnArr['Action'] = "0";
...
if ($type == "DisplayUserOrganizationProfile") {
    ...
    $iUserProfileId = isset($_REQUEST["iUserProfileId"]) ? $_REQUEST["iUserProfileId"] : '';
    if(isset($userDetailsArr['register_user_'.$iUserId])){
        $userdetail = $userDetailsArr['register_user_'.$iUserId];
    }else{
        $userdetail = $obj->MySQLSelect("SELECT *,iUserId as iMemberId FROM register_user WHERE iUserId='".$iUserId."'");
        $userDetailsArr['register_user_'.$iUserId] = $userdetail;
    }
    //$sql = "SELECT concat(vName,' ',vLastName) as senderName,vEmail,vPhone,vPhoneCode,vLang from  register_user  WHERE iUserId ='" . $iUserId . "'";
    //$userdetail = $obj->MySQLSelect($sql);
    $userdetail[0]['senderName'] = $userdetail[0]['vName']." ".$userdetail[0]['vLastName'];
    $vLang = $userdetail[0]['vLang'];
    if ($vLang == "" || $vLang == NULL) {
        //Added By HJ On 18-07-2020 For Optimize language_master Table Query Start
        $vLang = $LANG_OBJ->FetchDefaultLangData("vCode");
...
function sendSMSBookForSomeOneElse($vTripPaymentMode, $bokingData, $iTripId, $passangerData, $VehicleData, $DriverVehicleMake, $DriverVehicleModel, $Data_vehicle) {
    global $POOL_ENABLE, $PACKAGE_TYPE, $BOOK_FOR_ELSE_ENABLE, $SITE_ISD_CODE, $vLangCode,$languageLabelsArr, $COMM_MEDIA_OBJ;
    if (isset($bokingData[0]['vBookSomeOneNumber']) && $bokingData[0]['vBookSomeOneNumber'] != "" && strtoupper($PACKAGE_TYPE) == "SHARK") {
        ...
        $maildata1['PAYMENT_MODE'] = $vTripPaymentMode == "Cash" ? $languageLabelsArr['LBL_CASH_TXT'] : ($vTripPaymentMode == "Card" ? $languageLabelsArr['LBL_CARD'] : $languageLabelsArr['LBL_WALLET_TXT']);
        ...
        if(!empty($flyStationData[0]['iFromStationId']) && !empty($flyStationData[0]['iToStationId'])) {
            $message_layout = $COMM_MEDIA_OBJ->GetSMSTemplate("BOOK_FOR_SOMEONE_ELSE_SMS_FLY", $maildata1, "", $vLangCode);
        } else {
            $message_layout = $COMM_MEDIA_OBJ->GetSMSTemplate("BOOK_FOR_SOMEONE_ELSE_SMS", $maildata1, "", $vLangCode);
        }

        $phonecode = $passangerData[0]['vPhoneCode'];

        $result = $COMM_MEDIA_OBJ->SendSystemSMS($vBookSomeOneNumber, $phonecode, $message_layout); //added by SP for sms function change on 12-07-2019
    }
}

##############################Added By HJ On 22-03-2019 For Send SMS of Book For Someelse End#####################################

function sendSMSBookFromWeb($vTripPaymentMode, $bokingData, $iTripId, $passangerData, $VehicleData, $DriverVehicleMake, $DriverVehicleModel, $Data_vehicle) {
    global $vLangCode,$languageLabelsArr, $COMM_MEDIA_OBJ;

    $bookerName = $languageLabelsArr['LBL_ADMIN'];  
    if($bokingData[0]['eBookingFrom'] == "Admin") {
        $bookerName = $languageLabelsArr['LBL_ADMIN'];  
    } elseif ($bokingData[0]['eBookingFrom'] == "Company") {
        $bookerName = $languageLabelsArr['LBL_COMPANY_SIGNIN'];
    }
    ...
    $phonecode = $passangerData[0]['vPhoneCode'];
    $phoneno = $passangerData[0]['vPhone'];

    $result = $COMM_MEDIA_OBJ->SendSystemSMS($phoneno, $phonecode, $message_layout); //added by SP for sms function change on 12-07-2019
}
?>
