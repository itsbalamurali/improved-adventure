...
$eRestrictType = $_POST['eRestrictType'] ?? 'All';
$eType = $_POST['eType'] ?? 'Disallowed';
...
if (isset($_POST['submit'])) {
    if ('Add' === $action && !$userObj->hasPermission('create-restricted-area')) {
        $_SESSION['success'] = 3;
    ...
    if ('Edit' === $action && !$userObj->hasPermission('edit-restricted-area')) {
        $_SESSION['success'] = 3;
...
<!--[if !IE]><!--> <html lang="en"> <!--<![endif]-->
	...
	<head>
		<meta charset="UTF-8" />
		<title>Admin | Restricted Area <?php echo $action; ?></title>
		...
		<div id="wrap">
			...
			<div id="content">
				<div class="inner">
					<div class="row">
						<div class="col-lg-12">
							<h2><?php echo $action; ?> Restricted Area</h2>
					...
					<div class="body-div">
						<div class="form-group">
							<?php if (1 === $success) { ?>
								...
								<?php if ('UberX' === $APP_TYPE || $MODULES_OBJ->isOnlyDeliverAllSystem()) { ?>
								...
								<div class="row">
								 	...
								 	<div class="col-lg-3">
									  	<select class="form-control" name = 'eRestrictType' id="eRestrictType" >
										   <option value="All" <?php if ('All' === $eRestrictType) { ?> selected <?php } ?> >All</option>
										   <option value="Pick Up" <?php if ('Pick Up' === $eRestrictType) { ?> selected <?php } ?>>Pick Up</option>
										   <option value="Drop Off" <?php if ('Drop Off' === $eRestrictType) { ?> selected <?php } ?>>Drop Off</option>
									  	</select>
								 	</div>
							 	</div>
								<?php } ?>
								...
								<div class="row">
								 ...
								 <div class="col-lg-3">
									  <select class="form-control" name = 'eType' id="eType" >
										   <option value="Disallowed" <?php if ('Disallowed' === $eType) { ?> selected <?php } ?>>Disallowed</option>
										   <option value="Allowed" <?php if ('Allowed' === $eType) { ?> selected <?php } ?>>Allowed</option>
									  </select>
								 </div>
								</div>
								...
								<div class="row">
									<div class="col-lg-12">
										<?php if (('Edit' === $action && $userObj->hasPermission('edit-restricted-area')) || ('Add' === $action && $userObj->hasPermission('create-restricted-area'))) { ?>
											<input type="submit" class="btn btn-default" name="submit" id="submit" value="<?php if ('Add' === $action) {?><?php echo $action; ?> Area<?php } else { ?>Update<?php } ?>" >
										<?php } ?>
...
function checkareaexist(iLocationId) {
	...
	var ajaxData = {
	    'URL': '<?php echo $tconfig['tsite_url_main_admin']; ?>ajax_check_restricted_area.php',
	    'AJAX_DATA': 'iLocationId=' + iLocationId + '&restricted_id='+ restricted_id,
	};
