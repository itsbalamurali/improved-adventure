...
// Update eStatus
$iCancelReasonId = $_GET['iCancelReasonId'] ?? '';
$status = $_GET['status'] ?? '';
$AllowedCharge = $_GET['AllowedCharge'] ?? '';
// sort order
$flag = $_GET['flag'] ?? '';
$id = $_GET['id'] ?? '';
...
if (0 !== $id) {
    if ('up' === $flag) {
        ...
        $order_data = $sel_order[0]['iSortId'] ?? 0;
        $val = $order_data - 1;
        if ($val > 0) {
    ...
    for ($i = 0; $i < $count_all; ++$i) {
        ...
        ?>
															<tr class="gradeA">
																...
																<!-- <td><?php echo $eAllowedCharge; ?></td> -->
																<td width="10%" align="center">
																	<a href="cancel.php?iCancelReasonId=<?php echo $iCancelReasonId; ?>&AllowedCharge=<?php echo ('Yes' === $eAllowedCharge) ? 'No' : 'Yes'; ?>">

																		<button class="btn">
																			<i class="<?php echo ('Yes' === $eAllowedCharge) ? 'icon-eye-open' : 'icon-eye-close'; ?>"></i> <?php echo $eAllowedCharge; ?>
																		</button>
																	</a>
																</td>
                                                           		<?php if ($userObj->hasPermission('update-status-cancel-reasons')) { ?>
																<td width="10%" align="center">
																	<a href="cancel.php?iCancelReasonId=<?php echo $iCancelReasonId; ?>&status=<?php echo ('Active' === $eStatus) ? 'Inactive' : 'Active'; ?>">
																...
																<td width="10%" align="center">
																	<?php if (1 !== $iDisplayOrder) { ?>
																		<a href="cancel.php?id=<?php echo $iCancelReasonId; ?>&flag=up">
																			<button class="btn btn-warning">
																				<i class="icon-arrow-up"></i>
																			</button>
																		</a>
																		<?php } if ($iDisplayOrder !== $count_all) { ?>
																		<a href="cancel.php?id=<?php echo $iCancelReasonId; ?>&flag=down">
																			<button class="btn btn-warning">
																...
																<td width="10%" align="center">
																	<a href="cancel_action.php?id=<?php echo $iCancelReasonId; ?>">
																...
																<?php if ($userObj->hasPermission('delete-cancel-reasons')) { ?>
																	<td width="10%" align="center">
																		...
																		<form name="delete_form" id="delete_form" method="post" action="" onSubmit="return confirm_delete()" class="margin0">
																			<input type="hidden" name="hdn_del_id" id="hdn_del_id" value="<?php echo $iCancelReasonId; ?>">
