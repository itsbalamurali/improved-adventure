...
if ('' !== $keyword) {
    if ('' !== $option) {
        if (str_contains($option, 'status')) {
            $ssql .= ' AND '.stripslashes($option)." LIKE '".clean($keyword)."'";
        } else {
            $ssql .= ' AND '.stripslashes($option)." LIKE '%".clean($keyword)."%'";
        }
    } else {
        $permission_name = str_replace(['-', '_', ' ', '/', '\\'], '-', clean($keyword));

        if ('' !== $eStatus) {
            $ssql .= " AND ap.permission_name LIKE '%".$permission_name."%' AND ap.status = '".$eStatus."'";
        } else {
            $ssql .= " AND (ap.permission_name LIKE '%".$permission_name."%' OR ap.status LIKE '%".clean($keyword)."%')";
        }
    }
} else {
    if ('' !== $eStatus) {
                              ...
                              <tbody>
                                <tr>
                                    ...
                                    <select name="option" id="option" class="form-control">
                                          ...
                                          <option value="" <?php if ('permission_name' === $option) {
                                              echo 'selected';
                                                ...
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th width="20%"><a href="javascript:void(0);" onClick="Redirect(1,<?php if ('1' === $sortby) {
                                                            echo $order;
                                                        } else { ?>0<?php } ?>)">Permission Name <?php if (1 === $sortby) {
                                                        ...
                                                        <th width="10%" align="center" style="text-align:center;"><a href="javascript:void(0);" onClick="Redirect(2,<?php if ('2' === $sortby) {
                                                            echo $order;
                                                ...
                                                <tbody>
                                                    ...
                                                    if (!empty($data_drv)) {
                                                        for ($i = 0; $i < count($data_drv); ++$i) {
                                                            $default = '';
                                                            if ('Yes' === $data_drv[$i]['eDefault'] || ($_SESSION['sess_iAdminUserId'] === $data_drv[$i]['id'])) {
                                                                $default = 'disabled';
                                                            } ?>
                                                    <tr class="gradeA">
                                                        ...
                                                        <td><?php echo $data_drv[$i]['permission_name']; ?></td>
                                                        <td><?php echo $data_drv[$i]['group_name']; ?></td>
                                                        <td><?php echo $data_drv[$i]['display_group_name']; ?></td>

                                                        <td align="center" style="text-align:center;">
                                                                <?php if ('Active' === $data_drv[$i]['status']) {
                                                                    $dis_img = 'img/active-icon.png';
                                                                } elseif ('Inactive' === $data_drv[$i]['status']) {
                                                                    $dis_img = 'img/inactive-icon.png';
                                                                } elseif ('Deleted' === $data_drv[$i]['status']) {
                                                                    $dis_img = 'img/delete-icon.png';
                                                                }?>
                                                                <img src="<?php echo $dis_img; ?>" alt="image" data-toggle="tooltip" title="<?php echo $data_drv[$i]['status']; ?>">
                                                            </td>
                                                            <td align="center" style="text-align:center;" class="action-btn001">
                                                            <?php if (($_SESSION['sess_iAdminUserId'] === $data_drv[$i]['id']) || 'Yes' === $data_drv[$i]['eDefault']) {?>
                                                                    <a href="admin_permission_action.php?id=<?php echo $data_drv[$i]['id']; ?>" data-toggle="tooltip" title="Edit">
                                                                    <img src="img/edit-icon.png" alt="Edit">
                                                                </a>
                                                            <?php } else { ?>
                                                            <div class="share-button share-button4 openHoverAction-class" style="display: block;">
                                                                    <label class="entypo-export"><span><img src="images/settings-icon.png" alt=""></span></label>
                                                                   <div class="social show-moreOptions openPops_<?php echo $data_drv[$i]['id']; ?>">
                                                                        <ul>
                                                                            <li class="entypo-twitter" data-network="twitter"><a href="admin_permission_action.php?id=<?php echo $data_drv[$i]['id']; ?>" data-toggle="tooltip" title="Edit">
                                                                                <img src="img/edit-icon.png" alt="Edit">
                                                                            </a></li>
