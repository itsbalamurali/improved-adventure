...
$tableArr = ['pages_lang' => 'iPageId', 'email_templates_lang' => 'iEmailId'];
$tableArr = ['email_templates_lang' => 'iEmailId'];
$tableArr = ['delivery_fields' => 'iDeliveryFieldId'];
$tableArr = [];
...
foreach ($tableArr as $tableName => $pkIdField) {
    ...
    for ($d = 0; $d < count($table_data); ++$d) {
        ...
        foreach ($table_data[$d] as $key => $val) {
            if (str_contains($key, $engLangCode)) {
                $englishText = $val;
                ...
                if ('email_templates_lang' === $tableName) {
