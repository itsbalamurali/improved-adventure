...
if (isset($accessToken)) {
    if ('fblogin' === $ctype) {
        ...
        // Get logout url
        $logoutURL = $helper->getLogoutUrl($accessToken, $tconfig['tsite_url'].'fb-login/logout.php');

        /*echo "Full Name: ".$fbUser['name']."<br>";
        echo "First Name: ".$fbUser['email']."<br>";
        echo "Facebook ID: <a href='https://www.facebook.com/".$fbUser['id']."' target='_blank'>".$fbUser['id']."</a>";
        echo '<p><b>Logout from <a href="'.$logoutURL.'">Facebook</a></p>';die;*/
    ...
    if (str_contains($tconfig['tsite_url'], 'https')) {
    } else {
        $tconfig['tsite_url'] = str_replace('http', 'https', $tconfig['tsite_url']);
    }
    $loginUrl = $helper->getLoginUrl($tconfig['tsite_url'].'fb-login/fbconfig.php', $permissions);
