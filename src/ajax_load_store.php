...
<ul class="rest-listing" >
    ...
    if (!empty($Data)) {
        ...
        for ($i = 0; $i < count($Data); ++$i) {
            ...
            // $restaurant_status_arr = GetStoreWorkingHoursDetails($Data[$i]['iCompanyId'], $iUserId);
            ...
            $Data[$i]['Restaurant_fPricePerPer'] = formateNumAsPerCurrency($restPricePerPerson, '').' <div>'.ucfirst(strtolower($languageLabelsArr['LBL_PER_PERSON_TXT'])).'</div>';
            ...
            $Data[$i]['Restaurant_fMinOrderValue'] = formateNumAsPerCurrency($restMinOrdValue, '').' <div>'.ucfirst(strtolower($languageLabelsArr['LBL_MIN_SMALL'])).'. '.ucfirst(strtolower($languageLabelsArr['LBL_ORDER'])).'</div>';
            ?>
            <li <?php if ('closed' === strtolower($restaurantstatus)) { ?> class="rest-closed" <?php } ?>>
                <a href="<?php echo $tsite_url; ?>store-items?id=<?php echo $iCompanyId; ?>&order=<?php echo $fromOrder; ?>" data-status="<?php echo $Data[$i]['Restaurant_Open_And_Close_time']; ?>">
                    <?php
                    if (('rider' === strtolower($checkUser) || 'user' === strtolower($checkUser)) && 1 === $checkFavStore) {
                        ?>
            ...
            ?>    <div class="food-detail" title="<?php echo $Data[$i]['Restaurant_Cuisine']; ?>"><?php echo $Data[$i]['Restaurant_Cuisine']; ?></div></strong>
                        <div class="span-row">
    ...
    ?><div  align="center" class="show-msg-restaurant-listing">
            <h4><span style="color:#343434;"><b  style="margin:20px 0 0; padding:0px; float:left; width:100%;">
                        <?php /* if ($iServiceId != 1) { ?>
                          ...
                          <?php } */ ?>
                    </b><strong><?php echo $languageLabelsArr['LBL_NO_RECORD_FOUND']; ?></strong>
                    <!--<div><button class="btn" onclick="window.location.href = '<?php echo $redirect_location; ?>'"><?php echo $languageLabelsArr['LBL_CHANGE_LOCATION']; ?></button></div>-->
                </span> </h4></div>
    <?php } ?>
</ul>
                    </div>
