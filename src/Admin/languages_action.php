...
if ($action == 'Edit') {
    ...
    if (count($db_data) > 0) {
        foreach ($db_data as $key => $value) {
            ...
            //$$vValue = trim($value['vValue'], '"');//remove trim by SP in 25-01-2021 becoz when values contain " then it will be truncate.
            $$vValue = $value['vValue'];
                    ...
                    <div class="body-div">
                        <div class="form-group">
                            ...
                            <form method="post" name="_languages_form" id="_languages_form" action="">
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <label>Language Label Value <span class="red"> *</span></label>
                                    </div>
                                    <div class="col-md-6 col-sm-6">
                                        <input type="text" class="form-control <?= ($id == "") ?  'readonly-custom' : '' ?>" id="vValue_Default" name="vValue_Default" value="<?= htmlspecialchars($arrLang['vValue_'.$default_lang], ENT_QUOTES, 'UTF-8'); ?>" data-originalvalue="<?= htmlspecialchars($arrLang['vValue_'.$default_lang], ENT_QUOTES, 'UTF-8'); ?>" readonly="readonly" <?php if($id == "") { ?> onclick="editLangLabelValue('Add')" <?php } ?>>
                                    </div>
                                    <?php if($id != "") { ?>
                                    <div class="col-lg-2">
                                        <button type="button" class="btn btn-info" data-toggle="tooltip" data-original-title="Edit" onclick="editLangLabelValue('Edit')"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button>
                                    </div>
                                    <?php } ?>
                                </div>

                                <div  class="modal fade" id="lang_label_Modal" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                                    <div class="modal-dialog" >
                                        <div class="modal-content nimot-class">
                                            <div class="modal-header">
                                                <h4>
                                                    <span id="modal_action"></span> Language Label Value
                                                    <button type="button" class="close" data-dismiss="modal" onclick="resetToOriginalValue(this, 'vValue_')">x</button>
                                                </h4>
                                            </div>
                                            ...
                                            <div class="modal-body">
                                                ...
                                                ?>
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                <label>Language Label Value (<?= $vTitle; ?>) <?php echo $required_msg; ?></label>
                                                                
                                                            </div>
                                                            <div class="col-lg-12">
                                                                <input type="text" class="form-control" name="<?= $vValue; ?>" id="<?= $vValue; ?>" value="<?= htmlspecialchars($$vValue, ENT_QUOTES, 'UTF-8'); ?>" data-originalvalue="<?= htmlspecialchars($$vValue, ENT_QUOTES, 'UTF-8'); ?>" placeholder="<?= $vTitle; ?> Value">
                                                            ...
                                                            if (count($db_master) > 1) {
                                                                if($EN_available) {
                                                                    if($vCode == "EN") { ?>
                                                                    <div class="col-lg-12">
                                                                        <button type ="button" name="allLanguage" id="allLanguage" class="btn btn-primary" onClick="getAllLanguageCode('vValue_', 'EN');" style="margin-top: 10px">Convert To All Language</button>
                                                                        <button type ="button" name="allLanguage" id="allLanguage" class="btn btn-primary" style="margin: 10px 0 0 10px" onClick="CopyAllLanguageCode();">Copy <?= $def_lang_name ?> To All</button>
                                                                    </div>
                                                                <?php }
                                                                } else { 
                                                                    if($vCode == $default_lang) { ?>
                                                                    <div class="col-lg-12">
                                                                        <button type ="button" name="allLanguage" id="allLanguage" class="btn btn-primary" onClick="getAllLanguageCode('vValue_', '<?= $default_lang ?>');" style="margin-top: 10px">Convert To All Language</button>
                                                                        <button type ="button" name="allLanguage" id="allLanguage" class="btn btn-primary" style="margin: 10px 0 0 10px" onClick="CopyAllLanguageCode();">Copy <?= $def_lang_name ?> To All</button>
                                                                    </div>
                                                                <?php }
                                                                } ?>
                                            ...
                                            <div class="modal-footer" style="margin-top: 0">
                                                ...
                                                <div class="nimot-class-but" style="margin-bottom: 0">
                                                    <button type="button" class="save" style="margin-left: 0 !important" onclick="saveLangLabelValue()"><?= $langage_lbl['LBL_Save']; ?></button>
                                                    <button type="button" class="btn btn-danger btn-ok" data-dismiss="modal" onclick="resetToOriginalValue(this, 'vValue_')"><?= $langage_lbl['LBL_CANCEL_TXT']; ?></button>
                                                </div>
                                            </div>
...
<script type="text/javascript" language="javascript">
    ...
    function editLangLabelValue(action)
    {
        $('#modal_action').html(action);
        $('#lang_label_Modal').modal('show');
    }

    function saveLangLabelValue()
    {
        if($('#vValue_<?= $default_lang ?>').val() == "") {
        ...
        $('#vValue_Default').val($('#vValue_<?= $default_lang ?>').val());
        $('#lang_label_Modal').modal('hide');
    }
</script>
