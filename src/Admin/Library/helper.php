...
function get_admin_nav($menu_items, $lavel = 0)
{
    ...
    if (is_array($menu_items)) {
        foreach ($menu_items as $key => $menu_item) {
            if (isset($menu_item['visible']) && false === $menu_item['visible']) {
                continue;
            }

            $sectionHeader = (isset($menu_item['header']) && 1 === $menu_item['header']);
            ...
            } elseif (isset($menu_item['url']) && is_page($menu_item['url'])) {
                $has_active_menu = true;
                $class[] = 'active';
            }
            if (isset($menu_item['url']) && 'create_request.php' === strtolower($menu_item['url'])) {
            ...
            } elseif ($sectionHeader) {
                $inner_html = "<p clsss = 'menuHeader'>";
            } else {
    ...
    $ul_class = 0 === $lavel ? 'sidebar-menu' : 'treeview-menu menu_drop_down';
...
function is_page($name)
{
    ...
    return in_array(basename($_SERVER['REQUEST_URI']), $name, true);
}
...
function ArrayToExcelSheet(&$sheet, $array): void
{
...
function downloadExcel($doc): void
{
    ob_clean();
...
function setMessage($message, $type = 'success', $key = 'all'): void
{
...
function checked($value, $checked = '', $echo = true)
{
    $is_checked = false;
    if (is_array($checked)) {
        $is_checked = in_array($value, $checked, true);
    } else {
