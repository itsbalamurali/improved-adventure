...
class ImcStream
{
    ...
    public function stream_open($path, $mode)
    {
        ...
        if ($this->local) {
            ...
            if (isset($arr['data_limit']) && (-1 !== $arr['data_limit'])) {
                $this->limit = $arr['data_limit'] * 1_024;
            }
        }
        if ($this->seek) {
     ...
     */
    public static function fclose($path = null): void
    {
        if (null === $path) {
    ...
    protected function read($count)
    {
        if (-1 !== $this->limit && ($this->length + $count) > $this->limit) {
            $this->stream_close();
            if ($this->limit < 1_024 * 1_024) {
                $limit = number_format($this->limit / 1_024, 2).' KB';
            } else {
                $limit = number_format($this->limit / (1_024 * 1_024), 2).' MB';
            }
            error_reporting(error_reporting() & ~E_WARNING);
        ...
        if (-1 !== $this->timeout) {
            ...
            $microseconds = fmod($this->timeleft, 1) * 1_000_000;
