...
global $userObj;
if (!$userObj->hasPermission('view-driver-trackservice')) {
    $userObj->redirect();
}
//$defaultcurename = get_default_currency();
$script = 'TrackServiceDriver';
$iTrackServiceCompanyId = isset($_REQUEST['iTrackServiceCompanyId']) ? $_REQUEST['iTrackServiceCompanyId'] : '';
//Start Sorting
...
if (!empty($iTrackServiceCompanyId)) {
    $ssql .= " AND c.iTrackServiceCompanyId = {$iTrackServiceCompanyId} ";
}
if (ONLYDELIVERALL == 'Yes') {
    $sql = "SELECT COUNT(iDriverId) AS Total FROM register_driver rd WHERE 1 = 1  $eStatussql $ssql $ssql1 $dri_ssql";
} else {
    $sql = "SELECT COUNT(iDriverId) AS Total FROM register_driver rd LEFT JOIN track_service_company c ON rd.iTrackServiceCompanyId  = c.iTrackServiceCompanyId  WHERE 1 = 1  $eStatussql $ssql $ssql1 $dri_ssql";
}
///echo $sql;die;
...
$sql = "SELECT rd.iCompanyId,c.iTrackServiceCompanyId,rd.iDriverId,rd.vEmail,rd.tRegistrationDate,rd.vPhone,rd.vCode,rd.eStatus,rd.eIsFeatured,rd.vCountry,(SELECT count(dv.iDriverVehicleId) FROM driver_vehicle AS dv WHERE dv.iDriverId=rd.iDriverId AND dv.eStatus != 'Deleted' AND dv.iMakeId != 0 AND dv.iModelId != 0 AND dv.eType = 'TrackService') AS `count`,CONCAT(rd.vName,' ',rd.vLastName) AS driverName, c.vCompany,c.eStatus as cmp_status FROM register_driver rd LEFT JOIN track_service_company c ON rd.iTrackServiceCompanyId = c.iTrackServiceCompanyId WHERE 1=1 $eQuery $ssql $ssql1 $dri_ssql $ord LIMIT $start, $per_page";
...
<head>
    <meta charset="UTF-8"/>
    <title><?= $SITE_NAME ?> | <?= $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?></title>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="button" value="Reset" class="btnalt button11"
                                   onClick="window.location.href = 'track_service_driver.php'"/>
                        </td>
                        <td width="30%">
                            <?php if ($userObj->hasPermission('create-driver-trackservice')) { ?>
                                <a class="add-btn" href="track_service_driver_action.php" style="text-align: center;">
                        ...
                        <div class="admin-nir-export">
                            <div class="changeStatus col-lg-12 option-box-left">
                                <span class="col-lg-2 new-select001">
                                    <?php if ($userObj->hasPermission(['update-status-driver-trackservice', 'delete-driver-trackservice'])) { ?>
                                        <select name="changeStatus" id="changeStatus" class="form-control"
                                                onChange="ChangeStatusAll(this.value);">
                                            <option value="">Select Action</option>
                                            <?php if ($userObj->hasPermission('update-status-driver-trackservice')) { ?>
                                                <option value='Active' <?php
                                                if ($option == 'Active') {
                                                    echo "selected";
                                                }
                                            ...
                                            <?php if ($userObj->hasPermission('delete-driver-trackservice')) { ?>
                                                <option value="Deleted" <?php
                                                if ($option == 'Delete') {
                                                    echo "selected";
                                                }
                            ...
                            <form class="_list_form" id="_list_form" method="post" action="<?= $_SERVER['PHP_SELF'] ?>">
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th style="display: none" class="align-Left">Wallet Balance</th>
                                        <?php if ($doc_count != 0) { ?><?php //if($userObj->hasPermission('edit-provider-document')){   ?>
                                    ...
                                    if (!empty($data_drv)) {
                                        ...
                                        for ($i = 0; $i < count($data_drv); $i++) {
                                            ...
                                            <tr class="gradeA">
                                                ...
                                                <td><?php if ($userObj->hasPermission('view-providers')) { ?><a href="javascript:void(0);" onClick="show_driver_details('<?= $driverId; ?>')" style="text-decoration: underline;"><?php } ?><?= clearName($data_drv[$i]['driverName']); ?><?php if ($userObj->hasPermission('view-providers')) { ?></a> <?php } ?></td>
                                                ...
                                                <td>
                                                    <?php if (!empty($data_drv[$i]['vPhone'])) { ?>
                                                        (+<?= $data_drv[$i]['vCode']; ?>)
                                                        <?= clearPhone($data_drv[$i]['vPhone']); ?><?php } ?>
                                                </td>
                                                <?php
                                                if ($APP_TYPE != "UberX") {
                                                    if ($data_drv[$i]['count'] != 0) {
                                                        ?>
                                                        <td align="center">
                                                            <?php if ($userObj->hasPermission('view-driver-vehicle-trackservice')) { ?>
                                                                <a href="track_service_driver_vehicle.php?&actionSearch=1&iDriverId=<?= $driverId; ?>"
                                                ...
                                                ?>
                                                <td style="text-align: center"><?= DateTime($data_drv[$i]['tRegistrationDate'], 'No') ?></td>
                                                ...
                                                <td style="text-align: center">
                                                    ...
                                                    <?php $newUrl2 = "driver_document_action.php?id=" . $driverId . "&action=edit&user_type=tracking_driver";
                                                

                                                    ?>


                                                    <a href="<?= $newUrl2; ?>" data-toggle="tooltip" title="Edit <?= $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> Document" class="adminProviderDocumentClass" onclick="<?php if($MODULES_OBJ->isEnableServiceTypeWiseProviderDocument() == "Yes") { ?> alert('Please Add the services you will be offering'); <?php } ?> ">
                                                ...
                                                <td align="center" class="action-btn001">
                                                    <div class="share-button openHoverAction-class"
                                                        ...
                                                        <div class="social show-moreOptions for-five openPops_<?= $driverId; ?>">
                                                            <ul>
                                                                <?php if($userObj->hasPermission('edit-driver-trackservice')) { ?>
                                                                <li class="entypo-twitter" data-network="twitter">
                                                                    <a href="track_service_driver_action.php?id=<?= $driverId; ?>"
                                                                       data-toggle="tooltip" title="Edit">
                                                                        <img src="img/edit-icon.png" alt="Edit">
                                                                    </a>
                                                                </li>
                                                                <?php } ?>
                                                                <?php if ($data_drv[$i]['eDefault'] != 'Yes') { ?><?php if ($userObj->hasPermission('update-status-driver-trackservice')) { ?>
                                                                ...
                                                                <?php } ?><?php if ($userObj->hasPermission('delete-driver-trackservice') && $eStatus != 'Deleted') { ?>
                                                                    <li class="entypo-gplus" data-network="gplus">
                                                                        <a href="javascript:void(0);"
...
<!--END MAIN WRAPPER -->
<form name="pageForm" id="pageForm" action="action/track_service_driver.php" method="post">
            ...
            <form class="form-horizontal" id="add_money_frm" method="POST" enctype="multipart/form-data" action=""
                ...
                <div class="nimot-class-but">
                    <input type="button" onClick="check_add_money();" class="save" id="add_money"
                           name="<?= $langage_lbl_admin['LBL_save']; ?>" value="<?= $langage_lbl_admin['LBL_Save']; ?>">
    ...
    function show_driver_details(driverid) {
        ...
        if (driverid != "") {
            ...
            var ajaxData = {
                ...
                'AJAX_DATA': "iDriverId=" + driverid + "&trackingCompany=1",
