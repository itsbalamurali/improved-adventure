...
$iBaseFare = $_REQUEST['iBaseFare'] ?? '';
$fPricePerKM = $_REQUEST['fPricePerKM'] ?? '';
$fPricePerMin = $_REQUEST['fPricePerMin'] ?? '';
$iMinFare = $_REQUEST['iMinFare'] ?? '';
$fCommision = $_REQUEST['fCommision'] ?? '';
$tot_time = $_REQUEST['time'] ?? '';
$tot_distance = $_REQUEST['distance'] ?? '';
$iCountryId = $_REQUEST['iCountryId'] ?? '';
$iVehicleTypeId = $_REQUEST['iVehicleTypeId'] ?? '';

$default_currency = symbol_currency();
...
if ('' !== $iBaseFare && '' !== $booking_date) {
    ...
    $Data = GetAllSurgePriceDetails($iVehicleTypeId, $booking_date);
    ...
    $tot_distance = ('Miles' === $eUnit) ? round($tot_distance / 1.6, 2) : $tot_distance;
    ...
    $total_pick_surge_diff = ($PickFactor > 1 && 'Active' === $PickStatus) ? number_format(($PickFactor * $total_fare) - $total_fare, 2, '.', '') : '0.00';
    $total_night_surge_diff = ($NightFactor > 1 && 'Active' === $NightStatus) ? number_format(($NightFactor * $total_fare) - $total_fare, 2, '.', '') : '0.00';

    $PickSurgePrice = '0.00';
    $NightSurgePrice = '0.00';
    $GenerateFare = $total_fare;
    if ('Yes' === $PickSurgeApplied) {
        $GenerateFare = $total_fare + $total_pick_surge_diff;
        $PickSurgePrice = $GenerateFare;
    } elseif ('Yes' === $NightSurgeApplied) {
