...
function getCallMaskConfigNumber() {
    global $_REQUEST, $obj, $DEFAULT_COUNTRY_CODE_WEB, $tconfig, $CALLMASKING_ENABLED;
...
function calculateAdditionalTime($startDate, $endDate, $rentalTimeHours, $userlangcode, $value = "No") {
    global $LANG_OBJ;
...
function generateEstimatedRentalFare($data_calculation_arr) {
    ...
    $vCurrencyPassenger = $data_calculation_arr['vCurrencyPassenger'];
    $eRental_total_fare_value = $discountValue_rental = 0;
    if ($totalrental > 0) {
        $eRental = 'Yes';
        ...
        $eRental_total_fare_value = $total_rental_fare * $data_calculation_arr['priceRatio'];
        ...
        $eRental_total_fare = formateNumAsPerCurrency($total_rental_fare * $data_calculation_arr['priceRatio'],$vCurrencyPassenger);
    } else {
        $eRental = 'No';
        $eRental_total_fare = "";
    }
...
function isRentalEnable($iVehicleTypeId) {
    ...
    if (ENABLE_RENTAL_OPTION == 'Yes') {
        //Added By HJ On 07-07-2020 For Optimize rental_package Table Query Start
        if(isset($vehicleTypeDataArr['rental_package'])){
            $rentalPkgData = $vehicleTypeDataArr['rental_package'];
        } else {
            $rentalPkgData = $obj->MySQLSelect("SELECT count(iRentalPackageId) as totalrental,iVehicleTypeId FROM  `rental_package` WHERE 1=1 GROUP BY iVehicleTypeId");
            $vehicleTypeDataArr['rental_package'] = $rentalPkgData;
        }


        $typeDataArr =$rental_data= array();
        for($h=0;$h<count($rentalPkgData);$h++){
            $typeDataArr[$rentalPkgData[$h]['iVehicleTypeId']] = $rentalPkgData[$h]; 
        }


        if(isset($typeDataArr[$iVehicleTypeId])){
            $rental_data[] = $typeDataArr[$iVehicleTypeId];
        }
        //Added By HJ On 07-07-2020 For Optimize rental_package Table Query End
        //$checkrentalquery = "SELECT count(iRentalPackageId) as totalrental FROM  `rental_package` WHERE iVehicleTypeId = '" . $iVehicleTypeId . "'";
        //$rental_data = $obj->MySQLSelect($checkrentalquery);


        if (isset($rental_data[0]['totalrental']) && $rental_data[0]['totalrental'] > 0) {
            $eRental = 'Yes';
        }
...
if ($type == "getRentalPackages") {
    ...
    if ($vLang == "" || $vLang == NULL) {
        //Added By HJ On 29-09-2020 For Optimize language_master Table Query Start
        $vLang = $LANG_OBJ->FetchDefaultLangData("vCode");
        //Added By HJ On 29-09-2020 For Optimize language_master Table Query End
    ...
    for ($i = 0; $i < count($RentalPackagesData); $i++) {
        ...
        $RentalPackagesData[$i]['fPricePerKM'] = formateNumAsPerCurrency(($fPricePerKM * $priceRatio),$vCurrency);
        $RentalPackagesData[$i]['fPricePerHour'] = formateNumAsPerCurrency(($RentalPackagesData[$i]['fPricePerHour'] * $priceRatio),$vCurrency);
        ...
        $RentalPackagesData[$i]['fPrice'] = formateNumAsPerCurrency(($fPrice * $priceRatio),$vCurrency);
        ### Checking Promocode Discount ##
    }
    if ($totalcount > 0) {
        $returnArr['Action'] = "1";
...
if ($type == "getDriverVehicleDetails") {
    ...
    if ($iDriverVehicleId > 0) {
        ...
        if ($VehicleTypeIds != "") {
            ...
            $sql11 = "SELECT  vVehicleType_" . $vLang . " as vVehicleTypeName,iVehicleTypeId,vRentalAlias_" . $vLang . " as vRentalVehicleTypeName,iVehicleTypeId, vLogo,vLogo1, iPersonSize, tInfoText FROM vehicle_type WHERE iVehicleTypeId IN (" . $VehicleTypeIds . ") AND eType='Ride' AND eStatus='Active' $wherePool";
        } else {
            ...
            $sql_vehicle = "SELECT iVehicleTypeId,ePoolStatus FROM vehicle_type WHERE iLocationid IN (" . $GetVehicleIdfromGeoLocation . ") AND eType='Ride' AND eIconType != 'Ambulance' AND eStatus='Active'";
            ...
            $sql11 = "SELECT  vVehicleType_" . $vLang . " as vVehicleTypeName,vRentalAlias_" . $vLang . " as vRentalVehicleTypeName,iVehicleTypeId, vLogo,vLogo1, iPersonSize, tInfoText FROM vehicle_type WHERE iVehicleTypeId IN (" . $VehicleTypeIds_Str . ") AND eType='Ride' AND eStatus='Active'";
        }
        ...
        if (count($vCarType_Arr) > 0) {
            for ($i = 0; $i < count($vCarType_Arr); $i++) {
                ...
                $Fare_Single_Vehicle_Data = calculateApproximateFareGeneral($time, $distance, $vCarType_Arr[$i]['iVehicleTypeId'], $driverId, 1, "", "", "", 1, 0, 0, 0, "DisplySingleVehicleFare", "Driver", 1, "", $isDestinationAdded, $eFlatTrip, $fFlatTripPrice, $pickuplocationarr, $dropOffLocationArr);
                    ...
                    if (count($rental_data) > 0) {
                        if ($userType == 'Driver') {
                            ...
                            if (count($rentalData_Arr) > 0) {
                                ...
                                $Fare_Data[$i]['RentalSubtotal'] = formateNumAsPerCurrency(round($rental_data[0]['fPrice'] * $priceRatio, 1),$vCurrencyPassenger ,2);
                            }
                        } else {
                            $Fare_Data[$i]['eRental'] = 'Yes';
                        }
                ...
                $Fare_Data[$i]['SubTotal'] = formateNumAsPerCurrency($lastvalue1,$vCurrencyPassenger);
                $Fare_Data[$i]['VehicleFareDetail'] = $Fare_Single_Vehicle_Data;

                if($MODULES_OBJ->isEnableRestrictPassengerLimit())
                {
                    $Fare_Data[$i]['RESTRICT_PASSENGER_LIMIT_NOTE'] = str_replace('#PERSON_LIMIT#', $vCarType_Arr[$i]['iPersonSize'], $db_label[0]['vValue']);
                }

                if (!empty($vCarType_Arr[$i]['tInfoText'])) {
                    $tInfoText = json_decode($vCarType_Arr[$i]['tInfoText'], true);
                    $Fare_Data[$i]['tInfoText'] = $tInfoText['tInfoText_' . $vLang];
                }
...
if ($type == "StartHailTrip") {
    ...
    ### Checking For Pickup And DropOff Disallow ###
    $TripData = $obj->MySQLSelect("SELECT iTripId FROM `trips` WHERE iActive='On Going Trip'  AND iDriverId='" . $driverId . "'");
    if (count($TripData) > 0) {
        $returnArr['Action'] = "0";
    ...
    //Added By HJ On 29-09-2020 For Optimization currency Table Start
    if (!empty($vSystemDefaultCurrencyName)) {
        $default_currency = $vSystemDefaultCurrencyName;
    }else{
        $default_currency = get_value('currency', 'vName', 'eDefault', 'Yes', '', 'true');
    }
    //Added By HJ On 29-09-2020 For Optimization currency Table End
    ...
    $hailpassenger = $obj->MySQLSelect("SELECT * FROM `register_user` WHERE eHail = 'Yes' ORDER BY iUserId DESC");
    if (count($hailpassenger) > 0) {
        ...
        ## Update Trip Status ##
        // changed for rental
        $iTripID = InitiateHailTrip($iUserId, $driverId, $selectedCarTypeID, $PickUpLatitude, $PickUpLongitude, $PickUpAddress, $DestLatitude, $DestLongitude, $DestAddress, $fTollPrice, $vTollPriceCurrencyCode, $eTollSkipped, $iRentalPackageId);
    } else {
        $Data["vName"] = "Hail";
        ...
        //Added By HJ On 29-09-2020 For Optimize language_master Table Query Start
        $Data["vLang"] = $LANG_OBJ->FetchDefaultLangData("vCode");
        //Added By HJ On 29-09-2020 For Optimize language_master Table Query End
        $Data["eStatus"] = "Active";
        $Data["vCurrencyPassenger"] = $default_currency;
        ...
        if ($id > 0) {
            // changed for rental
            $iTripID = InitiateHailTrip($id, $driverId, $selectedCarTypeID, $PickUpLatitude, $PickUpLongitude, $PickUpAddress, $DestLatitude, $DestLongitude, $DestAddress, $fTollPrice, $vTollPriceCurrencyCode, $eTollSkipped, $iRentalPackageId);
            $iUserId = $id;
        }
    }
    ...
    if ($vFaceMaskVerifyImage_name != "") {
        ...
        $obj->MySQLQueryPerform("trips", $Data_update_trips, 'update', $where_trip);
    }
    ...
    //Added By HJ On 29-09-2020 For Optimize register_driver_ Table Query Start
    if(isset($userDetailsArr["register_driver_".$driverId])){
        $Data_vehicle = $userDetailsArr["register_driver_".$driverId];
    }else{
        $Data_vehicle = get_value('register_driver', '*,iDriverId as iMemberId', 'iDriverId', $driverId);
        $userDetailsArr["register_driver_".$driverId] = $Data_vehicle;
    }
    //Added By HJ On 29-09-2020 For Optimize register_driver_ Table Query End
    ...
    //Added By HJ On 29-09-2020 For Optimize trips Table Query Start
    if(isset($tripDetailsArr['trips_'.$iTripID])){
        $tripDetails = $tripDetailsArr['trips_'.$iTripID];
    }else{
        $tripDetails = $obj->MySQLSelect("SELECT * FROM trips WHERE iTripId='".$iTripID."'");
        $tripDetailsArr['trips_'.$iTripID] = $tripDetails;
    }
    //Added By HJ On 29-09-2020 For Optimize trips Table Query End
    ...
    $message_arr['iTripVerificationCode'] = $tripDetails[0]['iVerificationCode'];
    $message_arr['eSystem'] = "";
    ...
    if ($iTripID > 0) {
        ...
        $data['eFareType'] = $tripDetails[0]['eFareType'];
...
if ($type == "CheckVehicleEligibleForHail") {
    ...
    if ($enableCommisionDeduct == 'Yes' && ($APP_PAYMENT_MODE == "Cash" || $APP_PAYMENT_MODE == "Cash-Card")) {
        ...
        if ($WALLET_MIN_BALANCE > $user_available_balance) {
            ...
            if ($APP_TYPE == "UberX") {
                ...
                $returnArr['Msg'] = str_replace('####', formateNumAsPerCurrency(($WALLET_MIN_BALANCE * $ratio), $vCurrencyDriver), $languageLabelsArr['LBL_REQUIRED_MINIMUM_BALNCE_HAIL']);
            }
            setDataResponse($returnArr);
        }
    }
    ...
    if ($iDriverVehicleId > 0) {
        $vCarType = $obj->MySQLSelect("SELECT vCarType FROM driver_vehicle WHERE iDriverVehicleId = '" . $iDriverVehicleId . "'");
        $vehicleIds = explode(",", $vCarType[0]['vCarType']);
        $vehicleListIds = implode("','", $vehicleIds);
        $Vehiclelist = $obj->MySQLSelect("SELECT count(iVehicleTypeId) as total_ridevehicle FROM vehicle_type WHERE iVehicleTypeId IN ('" . $vehicleListIds . "') AND eType = 'Ride'");
...
if ($type == "displayWayBill") {
    ...
    if (count($tripData) > 0) {
        ...
        $CurrencyDriver = $driver_detail[0]['vCurrencyDriver'];
        ...
        if ($iRentalPackageId > 0) {
            ...
            $fPrice = formateNumAsPerCurrency(($PackageData[0]['fPrice'] * $priceRatio),$CurrencyDriver,2);
            ...
            if ($eFareType == "Regular") {
                ...
                $Rate = formateNumAsPerCurrency($iBaseFare,$CurrencyDriver) . " " . $languageLabelsArr['LBL_BASE_FARE_SMALL_TXT'] . " + " . formateNumAsPerCurrency($fPricePerMin,$CurrencyDriver) . " " . $languageLabelsArr['LBL_PRICE_PER_MINUTE_SMALL_TXT'] . " + " . formateNumAsPerCurrency($fPricePerKMNew,$CurrencyDriver) . " " . $DisplayDistanceTxt;
            }
            if ($eFareType == "Fixed") {
                $Rate = formateNumAsPerCurrency($fFixedFare,$CurrencyDriver) . " " . $languageLabelsArr['LBL_FIXED_FARE_TXT_ADMIN'];
            }
            if ($eFareType == "Hourly") {
                $Rate = formateNumAsPerCurrency($fPricePerHour,$CurrencyDriver) . " " . $languageLabelsArr['LBL_PER_HOUR_SMALL_TXT'];
            }
            if ($eFlatTrip == "Yes") {
                if ($fTripGenerateFare > 0) {
                    //$Rate = $vCurrencySymbol." ".$fTripGenerateFare;

                    $Rate = formateNumAsPerCurrency($fFlatTripPrice,$CurrencyDriver);
                } else {
        ...
        $tripArr['tTripRequestDate'] = converToTz($tripData[0]['tTripRequestDate'], $tripData[0]['vTimeZone'], $systemTimeZone);;
        ...
        $tripArr['PackageName'] = (!empty($pkgdata[0]['vName'])) ? $pkgdata[0]['vName'] : "";
