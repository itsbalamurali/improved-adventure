...
function removedCategoryData($catId): void
{
    global $obj;
    // echo "<pre>";
    $companyCusineArr = $deleteTableArr = [];
    ...
    $deleteTableArr[] = ['table' => 'service_categories', 'field' => 'iServiceId', 'ids' => $catId];
    ...
    if ('' !== $companyIds) {
        ...
        $deleteTableArr[] = ['table' => 'company', 'field' => 'iCompanyId', 'ids' => $companyIds];
        ...
        for ($co = 0; $co < count($getCompanyCuisineIds); ++$co) {
            if (!in_array($getCompanyCuisineIds[$co]['ccId'], $companyCusineArr, true)) {
        ...
        if ('' !== $foodMenuIds) {
            ...
            $deleteTableArr[] = ['table' => 'food_menu', 'field' => 'iFoodMenuId', 'ids' => $foodMenuIds];
            ...
            if ('' !== $menuItemIds) {
                ...
                $deleteTableArr[] = ['table' => 'menu_items', 'field' => 'iMenuItemId', 'ids' => $menuItemIds];
            }
        }
    }
    if ('' !== $cuisineIds) {
        $cuisineIds = trim($cuisineIds, ',');
        $deleteTableArr[] = ['table' => 'cuisine', 'field' => 'cuisineId', 'ids' => $cuisineIds];
        ...
        for ($cc = 0; $cc < count($getCompanyCuisineIds); ++$cc) {
            if (!in_array($getCompanyCuisineIds[$cc]['ccId'], $companyCusineArr, true)) {
    ...
    if (count($companyCusineArr) > 0) {
        ...
        $deleteTableArr[] = ['table' => 'company_cuisine', 'field' => 'ccId', 'ids' => $companyCuisineIds];
    }
