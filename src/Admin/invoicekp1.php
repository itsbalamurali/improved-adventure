...
$sql = "select tPlatitudes,tPlongitudes from trips_route_locations where iTripId = '" . $iTripId . "' AND eType='GoogleDirection'";
...
if(count($snap_route_latitudes_arr) > 0){
	...
	if(count($snap_route_latitudes_arr) > 100){
		for($i= 0; $i < count($snap_route_latitudes_arr); $i++){
			...
			if(empty($snap_route_locations_arr) || distanceByLocation($last_added_location_latitude, $last_added_location_longitude, $currentLatitude, $currentLongitude, "K") > 0.025){
...
$snap_route_locations_str = implode("|",$snap_route_locations_arr);
...
//$snap_to_road_api = "https://roads.googleapis.com/v1/nearestRoads?points=".$snap_route_locations_str."&interpolate=true&key=".$GOOGLE_SEVER_API_KEY_WEB;
// echo "<PRE>";print_r($snap_to_road_api);exit;
// echo "<BR/>::HERE::TotalCount:".count($snap_route_locations_arr);exit;
