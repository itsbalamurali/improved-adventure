...
final class TranslatorCacheTest extends TestCase
{
    ...
    public function testThatACacheIsUsed($debug): void
    {
        ...
        $translator->addResource($format, [$msgid => 'OK'], $locale);
        self::assertSame('OK', $translator->trans($msgid), '-> caching does not work in '.($debug ? 'debug' : 'production'));
    }
    ...
    public function testDifferentTranslatorsForSameLocaleDoNotOverwriteEachOthersCache($debug): void
    {
        ...
        $translator->addLoader($format, $this->createFailingLoader());
        $translator->addResource($format, [$msgid => 'OK'], $locale);
        self::assertSame('OK', $translator->trans($msgid), '-> the cache was overwritten by another translator instance in '.($debug ? 'debug' : 'production'));
    }
    ...
    public function testDifferentCacheFilesAreUsedForDifferentSetsOfFallbackLocales(): void
    {
        ...
        self::assertSame('bar (b)', $translator->trans('bar'));
    ...
    public function testPrimaryAndFallbackCataloguesContainTheSameMessagesRegardlessOfCaching(): void
    {
        ...
        self::assertFalse($catalogue->defines('bar')); // Sure, the "a" catalogue does not contain that message.

        $fallback = $catalogue->getFallbackCatalogue();
        self::assertTrue($fallback->defines('foo')); // "foo" is present in "a" and "b"
        ...
        $catalogue = $translator->getCatalogue('a');
        self::assertFalse($catalogue->defines('bar'));

        $fallback = $catalogue->getFallbackCatalogue();
        self::assertTrue($fallback->defines('foo'));
    }
    ...
    public static function runForDebugAndProduction(): iterable
    {
        return [[true], [false]];
    }
