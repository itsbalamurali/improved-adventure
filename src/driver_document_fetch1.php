    ...
    <form class="form-horizontal frm6" id="frm6" method="post" enctype="multipart/form-data" action="cx-driver_document_action.php?id=<?php echo $id[1]; ?>&master=<?php echo $id[0]; ?> " name="frm6">
        ...
        <div class="model-body">
                ...
                <div class="fileupload-preview thumbnail" style="width: 100%; height: 150px; ">
                    <?php if ('' === $db_user[0]['doc_file']) {
                        ...
                        if ('is_image' === $file_ext) { ?>
                            <img src = "<?php echo $tconfig['tsite_upload_driver_doc'].'/'.$id[1].'/'.$db_user[0]['doc_file']; ?>" alt ="<?php echo $db_user[0]['doc_name']; ?> not found"/>
                        <?php } else { ?>
                            <a href="<?php echo $tconfig['tsite_upload_driver_doc'].'/'.$id[1].'/'.$db_user[0]['doc_file']; ?>" target="_blank"><?php echo $db_user[0]['doc_name']; ?></a>
                        <?php } ?>
                    <?php } ?>
                </div>
                <div>
                    <span class="btn btn-file btn-success gen-btn"><span class="fileupload-new"><?php echo $langage_lbl['LBL_UPLOAD']; ?> <?php echo $vName; ?> <?php echo $langage_lbl['LBL_PHOTO']; ?></span>
                    <span class="fileupload-exists"><?php echo $langage_lbl['LBL_CHANGE']; ?></span>
                    <input type="file" name="driver_doc" /></span>
                    <a href="#" class="btn btn-danger fileupload-exists gen-btn" data-dismiss="fileupload"><?php echo $langage_lbl['LBL_REMOVE_TEXT']; ?></a>
            ...
            <?php if ('yes' === $db_user[0]['ex_status'] || 'yes' === $db_user1[0]['ex_status']) { ?>
                <div class="filters-column exp-date">
                    <label><?php echo $langage_lbl['LBL_EXP_DATE']; ?></label>

                    <div class="input-group input-append date" id="dp123" data-date="" data-date-format="yyyy-mm-dd">
                        <input class="form-control" type="text" id="dtpckrdLicenceExp" name="dLicenceExp" value="<?php if ('' !== $db_user[0]['ex_date']) {
        ...
        <div class="model-footer">
            <div class="button-block">
                <input type="submit" class="save save11 gen-btn" name="save" value="<?php echo $langage_lbl['LBL_Save']; ?>">
                <input type="button" class="cancel11 gen-btn" data-dismiss="modal" name="cancel" value="<?php echo $langage_lbl['LBL_CANCEL_TXT']; ?>">
            </div>
        </div>
    </form>
</div>
    ...
    $(document).ready(function() {
        ...
        messages: {
            driver_doc: {
                ...
                accept: '<?php echo addslashes($langage_lbl['LBL_UPLOAD_IMG_ERROR']); ?>'
            }
        }
    });
    });
