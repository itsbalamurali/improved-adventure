...
if (isset($_REQUEST['id'])) {
    ...
    if ((count($db_comp) > 0 || count($db_driver) > 0) && '' === $UserType) {
        if ('Deleted' === $db_comp[0]['eStatus'] || 'Deleted' === $db_driver[0]['eStatus']) {
            echo 0;
        } else {
            echo 0;
        }
    } elseif (count($db_river) > 0 && 'rider' === $UserType) {
        if ('Deleted' === $db_river[0]['eStatus']) {
            echo 0;
        } else {
            echo 0;
        }
