...
$vLocationName = $_POST['vLocationName'] ?? '';
...
if (isset($_POST['submit'])) {
    if ('Add' === $action && !$userObj->hasPermission('create-geo-fence-locations')) {
        $_SESSION['success'] = 3;
    ...
    if ('Edit' === $action && !$userObj->hasPermission('edit-geo-fence-locations')) {
        $_SESSION['success'] = 3;
...
<!--[if !IE]><!--> <html lang="en"> <!--<![endif]-->
    ...
    <head>
        <meta charset="UTF-8" />
        <title>Admin | Location <?php echo $action; ?></title>
        ...
        <div id="wrap">
            ...
            <div id="content">
                <div class="inner">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2><?php echo $action; ?> Location</h2>
                            <a href="location.php">
                            ...
                            <div class="col-lg-5">
                                <div class="form-group">
                                    <form method="post" name="location_form" id="location_form" action="">
                                        <input type="hidden" name="id" value="<?php echo $id; ?>" id="iLocationid"/>
                                        <input type="hidden" name="previousLink" id="previousLink" value="<?php echo $previousLink; ?>"/>
                                        <input type="hidden" name="backlink" id="backlink" value="location.php"/>
                                        <input type="hidden" class="form-control" name="tLatitude"  id="tLatitude" value="<?php echo $tLatitude; ?>">
                                        <input type="hidden" class="form-control" name="tLongitude"  id="tLongitude" value="<?php echo $tLongitude; ?>">
                                        <input type="hidden" class="form-control" name="cLatitude"  id="cLatitude" value="<?php echo $cLatitude; ?>">
                                        <input type="hidden" class="form-control" name="cLongitude"  id="cLongitude" value="<?php echo $cLongitude; ?>">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-6">
                                                <input type="text" class="form-control" name="vLocationName"  id="vLocationName" value="<?php echo $vLocationName; ?>" placeholder="Location Name" required>
                                            </div>
                                        </div>
                                        <?php
                                            ...
                                            <div class="col-lg-6">
                                                <select class="form-control" name ="iCountry" id="iCountry" required="required" onChange="getGeoCounty(this.value);">
                                                    ...
                                                    <?php for ($i = 0; $i < count($db_country); ++$i) {?>
                                                        <option value = "<?php echo $db_country[$i]['vCountry']; ?>" <?php if ($vCountry === $db_country[$i]['vCountry']) {?>selected<?php }?> ><?php echo $db_country[$i]['vCountry']; ?></option>
                                                    <?php }?>
                                                </select>
                                            </div>
                                        </div>
                                        <?php } else { ?>
                                                    <input type="hidden" name ="iCountry" id="iCountry" value="<?php echo $db_country[0]['vCountry']; ?>" >
                                                <?php } ?>
                                        <div class="row">
                                            <div class="col-lg-12">
                                            ...
                                            <div class="col-lg-6">
                                                <select class="form-control" name ="eFor" id="eFor" required="required" <?php if ('Edit' === $action) { ?>disabled <?php } ?>>
                                                    <option value="">Select Location For</option>
                                                    <option value ="Restrict" <?php if ('Restrict' === $eFor) {?>selected<?php }?>><?php echo $langage_lbl_admin['LBL_LOCATION_AREA_RESTRICTION']; ?></option>
                                                    <option value ="VehicleType" <?php if ('VehicleType' === $eFor) {?>selected<?php }?> ><?php echo $langage_lbl_admin['LBL_VEHICLE_SERVICE_TYPE_TXT']; ?></option>
                                                    <?php
                                                    if ('UberX' !== $APP_TYPE && 'Delivery' !== $APP_TYPE && 'STANDARD' !== $PACKAGE_TYPE && 'NO' === $onlyDeliverallModule && $MODULES_OBJ->isRideFeatureAvailable()) {
                                                        ?>
                                                        <option value ="FixFare" <?php if ('FixFare' === $eFor) {?>selected<?php }?> ><?php echo $langage_lbl_admin['LBL_FIXFARE_SMALL_TXT']; ?></option>
                                                        <?php
                                                    }
?>
                                                    <?php if ($MODULES_OBJ->isDeliverAllFeatureAvailable() || 'YES' === $onlyDeliverallModule) { ?>

                                                        <option value ="UserDeliveryCharge" <?php if ('UserDeliveryCharge' === $eFor) {?>selected<?php }?> ><?php echo $langage_lbl_admin['LBL_LOCATION_USER_DELIVERY_CHARGE']; ?></option>
                                                    <?php }?>
                                                    <?php
                                                    if ('Yes' === $ENABLE_AIRPORT_SURCHARGE_SECTION && 'UberX' !== $APP_TYPE && 'Delivery' !== $APP_TYPE && $MODULES_OBJ->isRideFeatureAvailable()) {
                                                        if ('NO' === $onlyDeliverallModule) {
                                                            ?>
                                                            <option value ="AirportSurcharge" <?php if ('AirportSurcharge' === $eFor) {?>selected<?php }?> >Airport Surcharge</option>
                                                            <?php
                                                        }
                                                    } if ($MODULES_OBJ->isEnableLocationwiseBanner()) { ?>
                                                        <option value ="Banner" <?php if ('Banner' === $eFor) {?>selected<?php }?>>Banner</option>
                                                    <?php } if ($MODULES_OBJ->isEnableLocationWisePromoCode()) { ?>
                                                        <option value="PromoCode" <?php if ('PromoCode' === $eFor) {?>selected<?php }?>>PromoCode</option>
                                                    <?php } ?>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row" style="display: none;">
                                            ...
                                            <div class="col-lg-6">
                                                <div class="make-switch" data-on="success" data-off="warning" id="mySwitch">
                                                    <input type="checkbox" name="eStatus" <?php echo ('' !== $id && 'Inactive' === $eStatus) ? '' : 'checked'; ?> id="eStatus"/>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <?php if (('Edit' === $action && $userObj->hasPermission('edit-geo-fence-locations')) || ('Add' === $action && $userObj->hasPermission('create-geo-fence-locations'))) {?>
                                                    <input type="submit" class=" btn btn-default" name="submit" id="submit"  value="<?php if ('Add' === $action) {?><?php echo $action; ?> Location<?php } else {?>Update<?php }?>" onclick="return IsEmpty();">
        ...
        <script src="//maps.google.com/maps/api/js?sensor=fasle&key=<?php echo $GOOGLE_SEVER_API_KEY_WEB; ?>&libraries=places,drawing" type="text/javascript"></script>
        <script>
...
<?php }?>
                                                        ...
                                                        // $('#pac-input').keyup(function (e) {
                                                            // buildAutoCompleteLat("pac-input", e.keyCode, "<?php echo $MIN_CHAR_REQ_GOOGLE_AUTO_COMPLETE; ?>","<?php echo $_SESSION['sess_lang']; ?>",'23.073892','72.5033506', function(latitude, longitude, address){
                                                             // // set map marker...
                                                                    // var myLatlng;
                                                        ...
                                                        show_locations();
                                                        $('#pac-input').keyup(function (e) {
                                                        buildAutoComplete("pac-input",e, "<?php echo $MIN_CHAR_REQ_GOOGLE_AUTO_COMPLETE; ?>","<?php echo $_SESSION['sess_lang']; ?>", function(latitude, longitude, address){
