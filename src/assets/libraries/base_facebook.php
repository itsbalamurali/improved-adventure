...
abstract class BaseFacebook
{
     ...
     *
     * @param bool $fileUploadSupport the file upload support status
     *
     * @return BaseFacebook
     */
     ...
     *
     * @return bool true if and only if the server supports file upload
     */
     ...
     * the user ID.
     *
     * @return int returns the UID of the Facebook user, or 0
    ...
    protected function _restserver($params)
    {
        ...
        if ('auth.expireSession' === $params['method']
            || 'auth.revokeAuthorization' === $params['method']) {
            $this->destroySession();
        }
     ...
     *
     * @return bool true if this is video post
     */
    protected function isVideoPost($path, $method = 'GET')
    {
        if ('POST' === $method && preg_match('/^(\\/)(.+)(\\/)(videos)$/', $path)) {
            return true;
        }
    ...
    protected function makeRequest($url, $params, $ch = null)
    {
        ...
        if (false === $result && empty($opts[CURLOPT_IPRESOLVE])) {
            ...
            if (preg_match($regex, curl_error($ch), $matches)) {
                if (16 === strlen(@inet_pton($matches[1]))) {
    ...
    protected function getApiUrl($method)
    {
        ...
        if (isset($READ_ONLY_CALLS[strtolower($method)])) {
            $name = 'api_read';
        } elseif ('video.upload' === strtolower($method)) {
            $name = 'api_video';
        }
    ...
    protected function getCurrentUrl()
    {
        ...
        $port =
          isset($parts['port'])
          && (('http://' === $protocol && 80 !== $parts['port'])
           || ('https://' === $protocol && 443 !== $parts['port']))
          ? ':'.$parts['port'] : '';
    ...
    protected function shouldRetainParam($param)
    {
        foreach (self::$DROP_QUERY_PARAMS as $drop_query_param) {
            if (str_starts_with($param, $drop_query_param.'=')) {
                return false;
            }
        }
     ...
     */
    protected function throwAPIException($result): void
    {
     ...
     */
    protected static function errorLog($msg): void
    {
     ...
     * @param array  $value
     */
    abstract protected function setPersistentData($key, $value): void;

    /**
     * Get the data for $key, persisted by BaseFacebook::setPersistentData().
     *
     * @param string $key     The key of the data to retrieve
     * @param bool   $default The default value to return if $key is not found
     *
     * @return mixed
     */
     ...
     * @param string $key
     */
    abstract protected function clearPersistentData($key): void;
