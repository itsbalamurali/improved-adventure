...
var apacheServer = eetase(http.createServer());
...
if(IS_USE_SSL.toLowerCase() === "yes"){
	...
	apacheServer = eetase(https.createServer(options));
}


let agServer = socketClusterServer.attach(apacheServer, agOptions);
...
(async () => {
  for await (let requestData of apacheServer.listener('request')) {
...
apacheServer.listen(SOCKETCLUSTER_PORT);
