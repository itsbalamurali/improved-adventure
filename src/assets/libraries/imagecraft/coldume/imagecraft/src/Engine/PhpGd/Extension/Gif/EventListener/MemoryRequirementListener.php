...
class MemoryRequirementListener implements EventSubscriberInterface
{
    ...
    public function verifyMemoryLimit(PhpGdEvent $event): void
    {
        ...
        $peak = memory_get_peak_usage(true) + 15 * 1_024 * 1_024 + 3 * ($pixels + $finalPixels);

        if (1_000_000 < $finalPixels || 57_000_000 < $totalPixels || $peak > $limit) {
            ...
            $this->extras['memory_approx'] = number_format($peak / (1_024 * 1_024), 2).' MB';
        }
    }
