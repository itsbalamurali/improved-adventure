...
if($keyword != ''){
    if($option != '') {
        ...
        $ssql.= " AND (concat(vName,' ',vLastName) LIKE '%".clean($keyword)."%' OR vEmail LIKE '%".clean($keyword)."%' OR vPhone LIKE '%".clean($keyword)."%' OR eStatus LIKE '%".clean($keyword)."%')";
    }
} else if(!empty($eStatus)) {
     $ssql.= " AND ".stripslashes($option)." LIKE '".clean($eStatus)."'";
}
