...
class DBConnection
{
     ...
     * @return    true/false
     */
    public function MySQLClose(): true
    {
        $conn = $this->CONN;
     ...
     * @return    array
     */
    public function MySQLSelect($sql = "", $cached = ""): false|array
    {
        //echo "<br><br>".$sql;
        if (empty($sql)) {
            return false;
        ...
        $conn = $this->CONN;
        // echo $this->getReformattedQuery($sql)."<br><br>";
    ...
    public function sql_query($sql = "")
    {
        ...
        if (strpos($sql, 'select') === false) {
            ...
            if (startsWith(strtoupper($sql), "INSERT INTO")) {
    ...
    {
        global $APP_TYPE, $SERVICE_CATEGORIES_IDS_ARR,$parent_ufx_catid,$iMasterVehicleCategoryId,$ServiceData, $THEME_OBJ;
        $final_sql_str = $sql_str;
        if(isset($_REQUEST['enablequery'])) {
            echo "Enablequery ==== ".$sql_str."<br>";
        }

        if(empty($SERVICE_CATEGORIES_IDS_ARR) && defined('serviceCategories')) {
            $getServiceCat = getCurrentActiveServiceCategoriesIds();
        ...
        if(empty($ServiceData) && defined('ServiceData')) {
        ...
        if ($THEME_OBJ->isCJXDoctorv2ThemeActive() == 'Yes') {
            $tables_arr = array("cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail", "help_detail_categories", "language_label", "package_type", "pages", "send_message_templates", "seo_sections", "vehicle_category", "vehicle_type", "app_launch_info");

            foreach ($tables_arr as $tables_arr_item) {
                $tb_name = $tables_arr_item . "_doctor";
                $query_tmp_match = preg_match("/\b" . $tables_arr_item . "\b/", $final_sql_str);
                if ($query_tmp_match && checkTableExistsDatabase($tb_name)) {
                    $final_sql_str = preg_replace("/\b" . $tables_arr_item . "\b/", $tb_name, $final_sql_str);
                }
            }
        } elseif ($THEME_OBJ->isMedicalServicev2ThemeActive() == 'Yes') {
            $tables_arr = array("banners", "cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail", "help_detail_categories", "language_label", "package_type", "pages", "send_message_templates", "seo_sections", "vehicle_category", "vehicle_type", "app_launch_info");

            foreach ($tables_arr as $tables_arr_item) {
                $tb_name = $tables_arr_item . "_medicalservice";
                $query_tmp_match = preg_match("/\b" . $tables_arr_item . "\b/", $final_sql_str);
                if ($query_tmp_match && checkTableExistsDatabase($tb_name)) {
                    $final_sql_str = preg_replace("/\b" . $tables_arr_item . "\b/", $tb_name, $final_sql_str);
                }
            }
        } elseif ($THEME_OBJ->isDeliveryKingThemeActive() == 'Yes' || $THEME_OBJ->isDeliveryKingXv2ThemeActive() == 'Yes') {
            $tables_arr = array("banners", "cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail", "help_detail_categories", "language_label", "package_type", "pages", "send_message_templates", "seo_sections", "vehicle_category", "vehicle_type", "app_launch_info");

            foreach ($tables_arr as $tables_arr_item) {
                $tb_name = $tables_arr_item . "_deliveryking";
                $query_tmp_match = preg_match("/\b" . $tables_arr_item . "\b/", $final_sql_str);
                if ($query_tmp_match && checkTableExistsDatabase($tb_name)) {
                    $final_sql_str = preg_replace("/\b" . $tables_arr_item . "\b/", $tb_name, $final_sql_str);
                }
            ...
            $tables_arr = array("cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail", "help_detail_categories", "home_content", "language_label", "language_label_other", "package_type", "pages", "rental_package", "send_message_templates", "seo_sections", "vehicle_category", "vehicle_type", "app_launch_info");
        ...
        } elseif (strtoupper($APP_TYPE) == "RIDE") {

            $tables_arr = array("cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail", "help_detail_categories", "home_content", "language_label", "language_label_other", "pages", "rental_package", "send_message_templates", "seo_sections", "vehicle_category", "vehicle_type", "app_launch_info");
            ...
            $tables_arr = array("cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail", "help_detail_categories", "home_content", "language_label", "language_label_other", "package_type", "pages", "send_message_templates", "seo_sections", "vehicle_category", "vehicle_type", "app_launch_info");
            ...
            $tables_arr = array("banners", "cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail", "help_detail_categories", "home_content", "language_label", "language_label_other", "pages", "send_message_templates", "seo_sections", "vehicle_category", "vehicle_type", "app_launch_info");
            if(strpos($final_sql_str, "master_vehicle_category") !== false) {
                //master_vehicle_category Table Found
            } else {
                foreach ($tables_arr as $tables_arr_item) {
                    $tb_name = $tables_arr_item . "_uberx";
                    //Added By HJ On 03-11-2020 For Get Data From Particular Service Table Start
                    if($parent_ufx_catid > 0) {
                        if(empty($iMasterVehicleCategoryId)) {
                            $catQuery = $this->MySQLSelect("SELECT iMasterVehicleCategoryId FROM `master_vehicle_category` WHERE iVehicleCategoryId = '".$parent_ufx_catid."'");
                            $iMasterVehicleCategoryId = $catQuery[0]['iMasterVehicleCategoryId'];
                        }
                        //echo "<pre>";print_r($iMasterVehicleCategoryId);die;
                        if($iMasterVehicleCategoryId > 0) {
                            $masterCatTable = $tb_name . "_".$iMasterVehicleCategoryId;
                            $query_tmp_match = preg_match("/\b" . $tables_arr_item . "\b/", $final_sql_str);
                            if ($query_tmp_match && checkTableExistsDatabase($masterCatTable)) {
                                $tb_name = $masterCatTable;
                            }
                        }
                    }
                    //Added By HJ On 03-11-2020 For Get Data From Particular Service Table End
                    $query_tmp_match = preg_match("/\b" . $tables_arr_item . "\b/", $final_sql_str);
                    if ($query_tmp_match && checkTableExistsDatabase($tb_name)) {
                        $final_sql_str = preg_replace("/\b" . $tables_arr_item . "\b/", $tb_name, $final_sql_str);
                    }
        ...
        } elseif (strtoupper($APP_TYPE) == "RIDE-DELIVERY-UBERX" && count($SERVICE_CATEGORIES_IDS_ARR) == 1 && strtoupper(ONLYDELIVERALL) == "YES") {

            $tables_arr = array("banners", "cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail_categories", "help_detail", "homecontentfood", "language_label", "language_label_other", "order_status", "pages", "send_message_templates", "seo_sections", "vehicle_type", "app_launch_info");

            $service_id = $SERVICE_CATEGORIES_IDS_ARR[0];
            $table_postfix = "";
            foreach ($ServiceData as $Service) {
                if($Service['iServiceId'] == $service_id) {
                    $table_postfix = strtolower($Service['vService']);
                }
            }

            foreach ($tables_arr as $tables_arr_item) {
                $tb_name = $tables_arr_item . "_" . $table_postfix;
                $tb_name_alt = $tables_arr_item . "_deliverall";

                $query_tmp_match = preg_match("/\b" . $tables_arr_item . "\b/", $final_sql_str);
                if ($query_tmp_match && checkTableExistsDatabase($tb_name)) {
                    $final_sql_str = preg_replace("/\b" . $tables_arr_item . "\b/", $tb_name, $final_sql_str);
                } elseif ($query_tmp_match && checkTableExistsDatabase($tb_name_alt)) {
                    $final_sql_str = preg_replace("/\b" . $tables_arr_item . "\b/", $tb_name_alt, $final_sql_str);
                }
            }
        } elseif (strtoupper($APP_TYPE) == "RIDE-DELIVERY-UBERX" && strtoupper(ONLYDELIVERALL) == "YES" /*&& count($SERVICE_CATEGORIES_IDS_ARR) > 1*/) {

            $tables_arr = array("banners", "vehicle_type", "cancel_reason", "document_master", "email_templates", "faqs", "faq_categories", "help_detail_categories", "help_detail", "homecontentfood", "language_label", "language_label_other", "order_status", "pages", "send_message_templates", "seo_sections", "vehicle_type", "app_launch_info");
            foreach ($tables_arr as $tables_arr_item) {
    ...
    {
        global $mongoConnectionObj, $IS_INHOUSE_DOMAINS, $mongoConnectionStr;
        
        if(empty($mongoConnectionObj)) {
            if($IS_INHOUSE_DOMAINS) {
                $mongoConnectionObj = new MongoDB\Client($mongoConnectionStr);
            } else {
                $mongoConnectionObj = (new MongoDB\Client);
            }
        }
    ...
    public function createMongoConnectionToDB()
    {
        ...
        if (empty($mongoDBObj)) {
            ...
            $mongoDBObj = ($this->getMongoClient())->$mongoDbName;
        }
        $this->addDataToMongoIfNotExist();
        return;
    }
    ...
    public function addDataToMongoIfNotExist()
    {
        ...
        if (!$this->isMongoDBConnected() || strtoupper(basename($_SERVER["SCRIPT_FILENAME"])) != "INDEX.PHP") {
            return;
        }

        $isCountryDataCreate = false;
        
        if($isCountryDataCreate) {
            // TImeZone DB
            $dataArr = array();
    ...
    public function fetchRecordsFromMongo($dataArr)
    {
        ...
        $obj_db_mongo = empty($dataArr['DB_NAME']) ? $mongoDBObj : (($this->getMongoClient())->$dataArr['DB_NAME']);
    ...
    {
        $mongoDbName = $this->DBASE;
        $collection = ($this->getMongoClient())->$mongoDbName->$tableName;
    ...
    {
        $mongoDbName = $dbName;
        $collection = ($this->getMongoClient())->$mongoDbName;
        $result = $collection->dropCollection($tableName);
        return $result;
    }

    public function isJsonTextStr($text_str)
    {
    ...
    public function getJsonFromAnArr($dataArr)
    {
