...
if ('delete' === $method && '' !== $iTrackServiceUserId) {
    if (!$userObj->hasPermission('delete-users-trackservice')) {
        $_SESSION['success'] = 3;
    ...
    header('Location:'.$tconfig['tsite_url_main_admin'].'track_service_user.php?'.$parameters);

    exit;
}
if ('' !== $iTrackServiceUserId && '' !== $status) {
    if (!$userObj->hasPermission('update-status-users-trackservice')) {
        $_SESSION['success'] = 3;
...
if ('' !== $checkbox && '' !== $statusVal) {
    if (!$userObj->hasPermission(['update-status-users-trackservice', 'delete-users-trackservice'])) {
        $_SESSION['success'] = 3;
        $_SESSION['var_msg'] = 'You do not have permission to change status of user';
    } else {
        if (SITE_TYPE !== 'Demo') {
